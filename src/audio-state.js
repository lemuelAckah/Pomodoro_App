...
function ensurePlayerState() {
  ...
  p.playlists =
    p.playlists && typeof p.playlists === "object" ? p.playlists : {}
  if (
    p.resume &&
    (typeof p.resume.trackId !== "string" || !Number.isFinite(+p.resume.pos))
  )
    p.resume = null
...
export function savePlayerStorage() {
  ...
  save("sf-player", {
    vol: p.volume,
    track:
      (state.songs || []).find((s) => s.playing)?.id ||
      state.playerTrack ||
      null,
    repeat: p.repeat,
    shuffle: p.shuffle,
    ...
    resume: p.resume,
    preShuffle:
      p.preShuffle && Array.isArray(p.preShuffle.queue) ? p.preShuffle : null,
  })
}
...
export function hydratePlayerFromStorage() {
  ...
  p.volume =
    typeof raw.volume === "number" ? Math.min(1, Math.max(0, raw.volume)) : 0.8
  ...
  p.playlists =
    raw.playlists && typeof raw.playlists === "object" ? raw.playlists : {}
  p.resume =
    raw.resume &&
    typeof raw.resume.trackId === "string" &&
    Number.isFinite(+raw.resume.pos)
      ? { trackId: raw.resume.trackId, pos: Math.max(0, +raw.resume.pos) }
      : null
  p.preShuffle =
    raw.preShuffle && Array.isArray(raw.preShuffle.queue)
      ? raw.preShuffle
      : null
  state.player = p
...
export function reorderQueue(fromIndex, toIndex) {
  ...
  if (
    fromIndex < 0 ||
    fromIndex >= p.queue.length ||
    toIndex < 0 ||
    toIndex >= p.queue.length
  )
    return
...
export function getResumePos(trackId) {
  ...
  if (
    p.resume &&
    p.resume.trackId === trackId &&
    Number.isFinite(+p.resume.pos)
  ) {
...
export function updatePlaylist(id, patch = {}) {
  ...
  if (patch.description !== undefined)
    pl.description = cleanPlaylistDesc(patch.description)
...
export function addToPlaylist(playlistId, songId) {
  ...
  if (pl.songIds.includes(songId))
    return { ok: false, error: "Already in this playlist" }
  if (pl.songIds.length >= MAX_PLAYLIST_TRACKS)
    return {
      ok: false,
      error: `Playlist is full (${MAX_PLAYLIST_TRACKS} tracks)`,
    }
...
export function moveInPlaylist(playlistId, from, to) {
  ...
  if (
    from < 0 ||
    from >= pl.songIds.length ||
    to < 0 ||
    to >= pl.songIds.length
  )
    return false
