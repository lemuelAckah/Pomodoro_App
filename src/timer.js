...
function recordFocusDay(sessionRef) {
  ...
  if (cloudRewards()) {
    ...
    try {
      secureStreak(`day:${today}:${sessionRef || "na"}`)
        .then((r) => {
          if (!r || !r.ok) return
          ...
          if (Number.isFinite(+r.current))
            state.streak.count = Math.max(1, +r.current)
          ...
          if (r.bonus > 0) {
            notify(
              `${sicon("fire")} ${r.current}-day streak · +${r.bonus} coins milestone!`,
            )
  ...
  if (state.streak.lastDate !== today) {
    if (!state.streak.lastDate || state.streak.lastDate === yesterday) {
      state.streak.count = state.streak.lastDate ? state.streak.count + 1 : 1
...
function gardenPlantMarkup(p) {
  ...
  return `<span class="garden-plant${wilted}" title="${p.rarity}${
    p.wilted ? " · wilted" : ""
  }" style="${color ? `color:${color};` : ""}${
    p.rarity === "legendary" && !p.wilted
      ? "filter:drop-shadow(0 0 6px rgba(201,138,31,.55));"
      : ""
  }">${p.wilted ? sicon("flower") : p.emoji}</span>`
}
...
function gardenMarkup() {
  ...
  return `<div class="card" style="margin-top:18px"><div class="section-row"><h2>Focus garden</h2><span class="tag">${state.garden.length} plants${
    rare ? ` · ${rare} rare+` : ""
  }</span></div>${
    plants.length
      ? `<div class="garden-grid">${plants.map(gardenPlantMarkup).join("")}</div><p class="muted" style="margin-top:10px">Every focus session plants one. Streaks of 7+ grow rares, 14+ grow legends. A broken streak wilts the newest bloom.</p>`
      : '<p class="muted">Finish a focus session to plant your first seed ' +
        sicon("sprout") +
        "</p>"
  }</div>`
}
...
function renderTimer() {
  ...
  target.innerHTML = `${viewHead("Focus desk", "A calm command centre for your next deep-work session.")}<div class="grid two"><div class="card timer-card"><div class="mode-switch">${Object.entries(
    ...
    .map(
      ([id, label]) =>
        `<button type="button" data-mode="${id}" class="${
  ...
  try {
    if (challengeLock()) {
      target
        .querySelectorAll("[data-template], #dur-min, #dur-sec, [data-set-dur]")
        .forEach((el) => {
          el.disabled = true
  ...
  try {
    ...
    if (target.querySelector("[data-mission-cd]")) {
      window.__sfMissionTick = setInterval(() => {
        if (
          state.tab !== "timer" ||
          !document.querySelector("[data-mission-cd]")
        )
          return
...
const ACHIEVEMENTS = [
  {
    ...
    emoji: sicon("sprout"),
    name: "First Session",
    desc: "Finish your very first focus session.",
    progress: () => ({ have: Math.min(state.sessions, 1), need: 1 }),
  },

  {
    ...
    progress: () => ({ have: Math.min(state.sessions, 10), need: 10 }),
  },

  {
    ...
    progress: () => ({ have: Math.min(state.sessions, 50), need: 50 }),
  },

  {
    ...
    progress: () => ({ have: Math.min(state.sessions, 100), need: 100 }),
  },

  {
    ...
    emoji: sicon("fire"),
    name: "Week Warrior",
    desc: "Grow a 7-day focus streak.",
    progress: () => ({
      have: Math.min(Math.max(state.streak.count, state.bestStreak || 0), 7),
      need: 7,
    }),
  },

  {
    ...
    emoji: sicon("comet"),
    name: "Unstoppable",
    desc: "Grow a 30-day focus streak.",
    progress: () => ({
      have: Math.min(Math.max(state.streak.count, state.bestStreak || 0), 30),
      need: 30,
    }),
  },

  {
    ...
    emoji: sicon("timer"),
    name: "10 Hours Deep",
    desc: "Accumulate 10 hours of total focus.",
    progress: () => ({
      have: Math.min(state.totalFocusMin, 600),
      need: 600,
      suffix: "m",
    }),
  },

  {
    id: "hours50",
    icon: "mountain",
    emoji: sicon("mountain"),
    name: "50 Hours Deep",
    desc: "Accumulate 50 hours of total focus.",
    progress: () => ({
      have: Math.min(state.totalFocusMin, 3000),
      need: 3000,
      suffix: "m",
    }),
  },

  {
    ...
    emoji: sicon("crown"),
    name: "Century Hours",
    desc: "Accumulate 100 hours of total focus.",
    progress: () => ({
      have: Math.min(state.totalFocusMin, 6000),
      need: 6000,
      suffix: "m",
    }),
  },

  {
    ...
    emoji: sicon("run"),
    name: "Marathon Day",
    desc: "Focus 120+ minutes in a single day.",
    progress: () => ({
      have: Math.min(biggestDay(), 120),
      need: 120,
      suffix: "m",
    }),
  },
...
const MASTERY_LADDER = [
  {
    ...
    badge: {
      id: "master-focus",
      name: "Master of Focus badge",
      category: "Badges",
      emoji: sicon("gem"),
      price: 0,
      description: "Earned through 250 focus sessions. Never sold.",
      tier: "Master",
      unbuyable: true,
    },
  },

  {
    ...
    progress: () => ({
      have: Math.min(Math.max(state.streak.count, state.bestStreak || 0), 60),
      need: 60,
    }),

    badge: {
      id: "grandmaster-resolve",
      name: "Grandmaster's Resolve badge",
      category: "Badges",
      emoji: sicon("crown"),
      price: 0,
      description: "Earned through a 60-day streak. Never sold.",
      tier: "Grandmaster",
      unbuyable: true,
    },
  },

  {
    ...
    progress: () => ({
      have: Math.min(state.totalFocusMin, 30000),
      need: 30000,
      suffix: "m",
    }),
    ...
    badge: {
      id: "mythic-mind",
      name: "Mythic Mind badge",
      category: "Badges",
      emoji: sicon("planet"),
      price: 0,
      description: "Earned through 500 hours of focus. Never sold.",
      tier: "Mythic",
      unbuyable: true,
    },
  },

  {
    ...
    progress: () => ({
      have: Math.min(Math.max(state.streak.count, state.bestStreak || 0), 365),
      need: 365,
    }),
    ...
    badge: {
      ...
      emoji: sicon("trophy"),
      price: 0,
      description: "Earned through a 365-day streak. Never sold.",
      tier: "Legendary",
      unbuyable: true,
    },
  },
...
function masteryLadderMarkup() {
  ...
  return `<div class="card mastery-ladder" style="margin-top:18px"><div class="section-row"><h2>${sicon("crown")} Mastery ladder</h2><span class="tag">${earned.length}/${MASTERY_LADDER.length} claimed</span></div><p class="muted">Four badges money can't buy — each one demands months of proof. Past bests count, so nothing you've already done is wasted.</p><div class="ladder">${MASTERY_LADDER.map(
    (m, i) => {
      ...
      return `<div class="ladder-rung${isEarned ? " earned" : ""}${
        i === MASTERY_LADDER.length - 1 ? " final" : ""
      }"><span class="ladder-node tier-${m.tier.toLowerCase()}">${m.emoji}</span><div class="ladder-body"><div class="section-row"><strong>${esc(m.name)}</strong><span class="tag rarity-${m.tier.toLowerCase()}">${esc(m.tier)}</span></div><small class="muted">${esc(m.desc)}</small>${
        isEarned
          ? `<span class="tag">claimed ${sicon("check")}</span>`
          : p
            ? `<div class="crew-track slim"><span class="crew-fill" style="width:${pct}%"></span></div><small class="muted">${p.have}/${p.need}${p.suffix} · ${pct}%</small>`
            : ""
      }<span class="tag unbuyable-tag">never sold · earn only</span></div></div>`
    },
  ).join("")}</div></div>`
}
...
function checkMasteryLadder() {
  ...
  MASTERY_LADDER.forEach((m) => {
    ...
    if (!(state.owned || []).some((o) => o && o.id === m.badge.id)) {
      state.owned = [
        ...(state.owned || []),
        { ...m.badge, owner: "me", boughtAt: Date.now() },
      ]
    }
...
function achievementProgress(a) {
  try {
    ...
    if (
      ...
      !Number.isFinite(+p.need) ||
      +p.need <= 0
    )
      return null

    return {
      have: Math.max(0, Math.round(+p.have)),
...
function achievementsCabinetMarkup() {
  ...
  return `<div class="card ach-cabinet" style="margin-top:18px"><div class="section-row"><h2>${sicon("trophy")} Achievement cabinet</h2><span class="tag">${earned.length}/${ACHIEVEMENTS.length} unlocked</span></div><p class="muted">Every badge below is earnable right inside your focus flow — here's exactly how.</p><div class="ach-grid">${ACHIEVEMENTS.map(
    (a) => {
      ...
      }"><span class="ach-tile-ico">${a.emoji}</span><div><strong>${esc(a.name)}</strong><small>${esc(a.desc)}</small>${
        isEarned
          ? `<span class="tag">unlocked ${sicon("check")}</span>`
          : p
            ? `<div class="crew-track slim"><span class="crew-fill" style="width:${pct}%"></span></div><small class="muted">${p.have}/${p.need}${p.suffix} · ${pct}%</small>`
            : ""
      }</div></div>`
...
function applyTemplate(t) {
  try {
    if (challengeLock()) {
      notify(
        "Challenge lock — your focus length is set by the race. Leave it to switch.",
      )
...
function applyCustomDuration(root) {
  try {
    if (challengeLock()) {
      notify(
        "Challenge lock — your focus length is set by the race. Leave it to change.",
      )
...
function techOfDayMarkup() {
  ...
  return `<div class="card tech-day" style="margin-top:18px"><div class="section-row"><h2>${sicon("sun")} Technique of the day</h2><span class="tag tech-day-tag">today's pick</span></div><div class="tech-day-body"><span class="tech-day-ico" aria-hidden="true">${x[2]}</span><div class="tech-day-copy"><strong>${x[1]}</strong><span class="muted">${esc(x[3])}</span><span class="tech-day-meta">${
    details.category ? `${esc(details.category)} · ` : ""
  }${esc(x[4])}</span></div><button type="button" class="primary tech-day-btn" data-today-tech="${x[0]}" title="Opens the full ${esc(x[1])} guide in the Techniques tab">Read &amp; try it →</button></div></div>`
}
  ...
  return `<div class="grid two" style="margin-top:18px"><div class="card"><div class="section-row"><h2>Records</h2><span class="tag">all time</span></div><div class="records-grid"><div><strong>${state.bestStreak || 0}${sicon("fire")}</strong><span>longest streak</span></div><div><strong>${biggestDay()}m</strong><span>biggest day</span></div><div><strong>${hours}h ${mins}m</strong><span>total focus</span></div><div><strong>${state.sessions}</strong><span>sessions</span></div></div><div class="section-row" style="margin-top:14px"><h3>Achievements</h3><span class="tag">${earned.length}/${ACHIEVEMENTS.length}</span></div><div class="ach-row">${earned.map((a) => `<span class="ach earned" title="${esc(a.name)}">${a.emoji}</span>`).join("")}${locked.map((a) => `<span class="ach locked" title="${esc(a.name)} — locked">${a.emoji}</span>`).join("")}</div></div><div class="card"><div class="section-row"><h2>Win journal</h2><span class="tag">${(state.focusLog || []).length}</span></div>${
    wins.length
      ? wins
          .map(
            (w) =>
              `<div class="win-row"><span>${sicon("check")}</span><div><p data-win-text="${w.id}">${esc(w.text)}</p><small class="muted">${new Date(w.at).toLocaleDateString()} · ${w.mins}m focus</small></div><span class="win-actions"><button type="button" class="icon-btn" data-win-edit="${w.id}" title="Edit win" aria-label="Edit win">${sicon("memo")}</button><button type="button" class="icon-btn" data-win-copy="${w.id}" title="Copy win" aria-label="Copy win">${sicon("clip")}</button><button type="button" class="icon-btn" data-win-del="${w.id}" title="Delete win" aria-label="Delete win">${sicon("trash")}</button></span></div>`,
          )
          .join("")
...
function editWin(id) {
  ...
  ta.onkeydown = (e) => {
    if (e.key === "Enter" && (e.ctrlKey || e.metaKey))
      modal.querySelector("[data-win-save]").click()
  }
}
...
async function copyWin(id) {
  ...
  try {
    ...
    try {
      ...
      notify("Could not copy — select the text manually")
    }
...
function bindWinActions(target) {
  $$("[data-win-edit]", target).forEach(
    (b) => (b.onclick = () => editWin(b.dataset.winEdit)),
  )

  $$("[data-win-copy]", target).forEach(
    (b) => (b.onclick = () => copyWin(b.dataset.winCopy)),
  )

  $$("[data-win-del]", target).forEach(
    (b) => (b.onclick = () => deleteWin(b.dataset.winDel)),
  )
}
...
function showCompletionCard(kind) {
  ...
  if (kind === "focus") {
    ...
    overlay.innerHTML = `<div class="modal complete-card"><div class="complete-emoji">${sicon("party")}</div><div class="eyebrow">Focus session complete</div><h2>Beautiful work.</h2><p class="muted">+${state.lastReward || 10 + 2 * state.streak.count} coins earned · ${state.sessions} total sessions · ${state.streak.count}-day streak</p><div class="score-line"><strong>${state.lastScore ?? 100}</strong><span> · ${scoreGrade(state.lastScore ?? 100)}</span></div><input class="input" id="accomp-input" placeholder="What did you accomplish? (optional)" aria-label="What did you accomplish" style="margin-top:12px"><div class="modal-actions" style="justify-content:center;margin-top:18px"><button type="button" class="ghost" data-complete-close>Back to desk</button><button type="button" class="primary" data-complete-next>Start ${mins}-min ${
      next === "long" ? "long break" : "break"
    }</button></div></div>`
  ...
  $("[data-break-spin]", overlay).onclick = () => {
    ...
    if (idea)
      idea.innerHTML = `Try this break: <strong>${randomBreakIdea()}</strong>`
  }
...
function updateTimerDom() {
  ...
  $$("[data-mission-cd]").forEach((el) => {
    ...
    if (t < sp.startsAt) {
      ...
      const s = Math.max(
        0,
        Math.round((sp.startsAt + sp.durationSec * 1000 - t) / 1000),
      )
...
function renderMiniTimer() {
  ...
  root.innerHTML = `<div class="mini-timer"><span class="mini-live off" data-mini-live title="Focus live — leaving this page resets the session">${sicon("rec")}</span><span class="eyebrow" data-mini-mode style="color:#b7d0bd">${
    state.sessionTech && techniques.find((y) => y[0] === state.sessionTech)
      ? `${techniques.find((y) => y[0] === state.sessionTech)[2]} `
      : ""
  }${modeLabels[state.mode]}</span><span class="mini-time" data-mini-time>${fmt(state.time)}</span><button type="button" class="mini-btn" data-mini-toggle>${
    state.running ? "Pause" : "Resume"
  }</button><button type="button" class="mini-btn ghost" data-mini-goto>Focus desk</button></div>`
...
function taskRow(task) {
  return `<div class="task ${
    task.done ? "done" : ""
  }" data-task-row="${task.id}"><button type="button" class="task-check ${
    task.done ? "done" : ""
  }" data-task-check="${task.id}" aria-pressed="${Boolean(task.done)}" title="${
    task.done ? "Mark as not done" : "Mark as done"
  }" aria-label="${
    task.done ? "Mark as not done" : "Mark as done"
  }"><span class="tc-box">${sicon("check")}</span></button><span class="task-text">${esc(task.text)}${
    task.desc ? `<br><small class="muted">${esc(task.desc)}</small>` : ""
  }</span><span class="task-meta">${task.pomodoros || 0} ◷</span><button type="button" class="ghost task-edit" data-edit-task="${task.id}">Edit</button><button type="button" class="delete" data-delete-task="${task.id}" title="Remove task">×</button></div>`
}
