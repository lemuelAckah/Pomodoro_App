...
function friendStatusRing(id, handle, name, stories) {
  ...
  return (
    `<button type="button" class="story-ring ${ringCls}" ${
      has ? `data-friend-stories="${esc(id)}"` : "disabled"
    } ${
      has
        ? `title="@${esc(handle)}'s status"`
        : `title="@${esc(handle)} has no status yet"`
    }>` +
  ...
  for (const s of cloudStories) {
    ...
    if (!groups.has(s.userId)) {
      groups.set(s.userId, {
        author: {
          id: s.userId,
          handle: s.handle || "member",
          name: s.name || s.handle || "member",
          photo: friendPhotoUrl(s.userId),
        },
...
function friendsWithStories() {
  return cloudStoryGroups().filter((g) =>
    cloudFriends.some((f) => f.id === g.author.id),
  )
}
...
function cloudFriendReqsById(id) {
  return (
    ...
    (() => {
      const r = cloudFriendReqs.find((x) => x.otherId === id)
      return r ? { id: id, handle: r.handle, name: r.name } : null
    })()
  )
}
...
function friendlyUploadError(err) {
  ...
  if (/phase 7 database update/i.test(msg))
    return "Cloud photos need the latest database update."
  ...
  if (
    /not signed in|sign in|session|permission|policy|not allowed|unauthorized|forbidden|owner or an admin|GROUP_FORBIDDEN|NOT_SIGNED_IN/i.test(
      msg,
    )
  )
...
function isTransientUploadError(err) {
  ...
  return /network|fetch|failed to fetch|timeout|econnreset|503|502|504/i.test(
...
function cloudSocialSig() {
  return JSON.stringify([
    cloudFriends.map((f) => f.id),
    cloudFriendReqs.map((r) => r.id + r.incoming),
    [...cloudBlocked].sort(),
  ])
}
...
function groupAvatarMarkup(g) {
  ...
  if (!path || !signedIn())
    return `<div class="group-logo">${g?.emoji || "●"}</div>`
...
async function refreshCloudGroups(force) {
  ...
  if (!force && Date.now() - cloudGroupsAt < 60000 && cloudGroups.length)
    return cloudGroups
  ...
  try {
    ...
    if (!error && Array.isArray(data)) {
      ...
      groupsBackendStatus = /Failed to fetch|network|NetworkError/i.test(
        error.message || "",
      )
        ? "offline"
        : "down"
...
async function refreshCloudSocial(force) {
  ...
  try {
    ...
    if (shipsFail) {
      friendsBackendStatus = /fetch|network/i.test(String(results[0].reason))
        ? "offline"
        : "down"
    ...
    const others = [
      ...new Set(
        rows
          .map((r) => (r.user_id === me ? r.friend_id : r.user_id))
          .filter(Boolean),
      ),
    ]
...
async function refreshCloudNotifCount(force) {
  ...
  if (!force && Date.now() - cloudNotifAt < 60000 && cloudNotifAt)
...
function restoreSectionAfterCall() {
  ...
  if (
    state.tab === snap.tab &&
    state.subtab === snap.subtab &&
    state.activeChat === snap.activeChat
  )
    return
...
function updateOngoingCallPill() {
  ...
  el.setAttribute(
    "aria-label",
    `Return to call with ${state.call.name || "study partner"}`,
  )
...
async function resolveCallTargets() {
  ...
  if (state.call?.peerId && state.call.peerId !== myId)
    targets.add(state.call.peerId)
    ...
    const handle = await createWebRtcPeer({
      ...
      onStateChange: (status) => {
        ...
        if (status === "failed" || status === "disconnected") {
          ...
          const isDm =
            state.call?.kind === "dm" || !(state.call?.extras || []).length
...
function ensureIncomingCallSubscription() {
  ...
  incomingCallSub = subscribeToIncomingCalls(state.user.id, {
    ...
    onCancelled: (row) => {
      ...
      const friend =
        cloudFriends.find((f) => f.id === row.initiator_id) ||
      ...
      if (pendingIncomingCall?.id === callId) {
        ...
        if (status === "declined" || status === "cancelled") {
          notifyCall(
            "Call cancelled",
            `${who} ended the ring before you answered`,
            "phone",
            `${callId}:cancelled`,
          )
      ...
      if (state.call && activeCallHistoryId === callId) {
        if (status === "declined") {
          ...
          notifyCall(
            status === "missed" ? "Missed call" : "Call cancelled",
            `${who} did not answer`,
            "phone",
            `${callId}:${status}`,
          )
...
function teardownCall(notifyEnd = true) {
  ...
  if (activeCallHistoryId) {
    ...
    const endWholeCall =
      !isGroupCall ||
      !keptInitiatorId ||
      keptInitiatorId === (state.user?.id || "")

    if (endWholeCall) {
      const endedStatus =
        callMeta?.status === "ringing" || callMeta?.status === "calling"
          ? notifyEnd
            ? "cancelled"
            : "missed"
          : "ended"
  ...
  if (callMeta) {
    ...
    const dur = callMeta.startedAt
      ? callDurationLabel(Date.now() - callMeta.startedAt)
      : ""
    ...
    if (notifyEnd) {
      if (wasLive && dur) {
        notifyCall(
          `${mode} ended`,
          `with ${callMeta.name} · ${dur}`,
          "phone",
          callMeta.callId ? `${callMeta.callId}:ended-note` : undefined,
        )

        toast("Call ended")
      } else if (
        callMeta.status === "ringing" ||
        callMeta.status === "calling"
      ) {
        notifyCall(
          "Call ended",
          `with ${callMeta.name}`,
          "phone",
          callMeta.callId ? `${callMeta.callId}:cancel-note` : undefined,
        )
...
async function startOutgoingCall(chatId, opts = {}) {
  ...
  state.call = isGroup
    ...
    : {
        ...
        peerId,
        kind: "dm",
        name: peer?.name || peer?.handle || "Study partner",
        photo: peerPhoto,
        emoji: "◉",
        color: "#47765a",
        extras: [],
  ...
  try {
    ...
    if (
      document.querySelector(
        ".modal-backdrop, [data-picker]:not([hidden]), .post-menu:not([hidden]), [data-chat-pop]:not([hidden]), [data-chat-extras-pop]:not([hidden]), .friend-pick.drop:not([hidden])",
      )
    )
      return true
  } catch {
  ...
  sprintTicker = setInterval(
    () => {
      ...
      ;(state.sprints || []).forEach((sp) => {
        ...
        if (
          !sp.result &&
          (sp.invites || []).some((i) => i.status === "pending") &&
          Math.random() < 0.03
        ) {
        ...
        const liveNow =
          now >= sp.startsAt && now < sp.startsAt + sp.durationSec * 1000
        ...
        if (now >= sp.startsAt && !sp.liveNotified) {
          ...
          if (lock) {
            notify(
              `🔒 “${sp.title}” is live, but your challenge lock holds the pace`,
            )
            ...
            notify(
              `${sicon("bolt")} Sprint live — ${Math.round(sp.durationSec / 60)} min focus started`,
            )
          } else {
            notify(
              sicon("bolt") +
                " Your sprint started — finish this session first",
            )
          }
        }
      ...
      ;(state.events || []).forEach((e) => {
        if (
          !e ||
          e.at < Date.now() ||
          !(e.invites || []).some((i) => i.status === "pending")
        )
          return
      ...
      $$("[data-crew-fill]").forEach((el) => {
        ...
        el.classList.toggle(
          "live",
          Boolean(member.live && (member.left ?? 0) > 0),
        )
        ...
        if (tag) {
          ...
          tag.innerHTML = done
            ? sicon("check") + " done"
            : `${member.left} sprint${member.left === 1 ? "" : "s"} left`
        }
      })

      $$("[data-mission-cd]").forEach((el) => {
        const sp = (state.sprints || []).find(
          (x) => x.id === el.dataset.missionCd,
        )
        ...
        el.textContent =
          t < sp.startsAt
            ? "starts " +
              new Date(sp.startsAt).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })
            : t < sp.startsAt + sp.durationSec * 1000
              ? "● LIVE " +
                fmtCountdown(sp.startsAt + sp.durationSec * 1000 - t)
              : "ended"
      })
...
function paintMessagesBadge(count) {
  ...
  const badge = count
    ? ` <span class="subnav-badge" data-msgs-unread>${
        count > 99 ? "99+" : count
      }</span>`
    : ""
...
function messagesStoryStrip() {
  ...
  return `<div class="msg-story-strip">${groups
    ...
    .map((g) =>
      friendStatusRing(g.author.id, g.author.handle, g.author.name, g.items),
    )
...
function bindFriendStoryRings(root) {
  $$("[data-friend-stories]", root).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        openStoryViewer(
          ...
          Math.max(
            0,
            items.findIndex((s) => !isSeen("cstory:" + s.id)),
          ),
        )
      }),
  )
...
function renderCommunity() {
  ...
  t.innerHTML = `${
    immersive
      ...
      : viewHead(
          "Community",
          "Find people who are learning what you are learning, make a group, and keep the conversation moving.",
        )
  }${
    immersive
      ? ""
      : `<div class="subnav">${[
          ...
          [
            "notifications",
            `Notifications${cloudNotifCount ? ` (${cloudNotifCount})` : ""}`,
          ],
  ...
  $$("[data-subtab]", t).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (
          leaving &&
          b.dataset.subtab !== "discover" &&
          b.dataset.subtab !== "status"
        ) {
          clearCloudStoryPhoto()

          // Leaving the editor discards unsent media/music but keeps the

          // plain-text draft (offline restore path) and the visibility pick.
...
function leaderboardMarkup() {
  ...
  return `<div class="card" style="margin-bottom:18px"><div class="section-row"><h2>This week's focus</h2><span class="tag">${total} min</span></div><div class="week-bars">${days.map((d) => `<div class="week-bar${d.today ? " today" : ""}" title="${d.min} min"><span style="height:${Math.round((d.min / max) * 100)}%"></span><small>${d.label}</small></div>`).join("")}</div><p class="muted" style="margin-top:10px">Best day ${best} min · ${
    delta >= 0 ? `+${delta}` : delta
  } min vs last week</p><div class="section-row" style="margin-top:12px"><h3>Global board</h3></div><p class="muted">${
    backendConfigured && state.user
      ? "Season board syncs with cloud accounts — invite friends from the Friends tab to race you."
      : sicon("globe") +
        " The global board unlocks with cloud accounts. Your week already counts — invite friends to race you."
  }</p><button type="button" class="ghost" data-invite-race>Copy race invite</button></div>`
}
...
function makeCrewPeer(name) {
  ...
  const left = Math.max(
    0,
    Math.min(total, Math.floor(Math.random() * (total + 1))),
  )
...
function sanitizeSprint(sp) {
  ...
  sp.invites = sp.invites.map((inv) =>
    typeof inv === "string"
      ? { id: "inv-" + uid(), username: inv, status: "pending" }
      : inv,
  )
...
function sprintCrewWithMe(sp) {
  ...
  if (!mine) {
    crew.unshift({
      ...
      pct:
        sp.result?.status === "done"
          ? 100
          : sessionInProgress() && state.sprintSession === sp.id
            ? 62
            : 12,

      live: Boolean(
        state.sprintSession === sp.id ||
          (state.running && state.mode === "focus"),
      ),
    })
  } else {
    mine.live = Boolean(
...
function crewPaceMarkup(sp) {
  ...
  return `<div class="crew-pace"><div class="crew-head"><span>${sicon("users")} Crew pace</span><span class="muted">${crew.filter((c) => (c.left ?? 0) === 0).length}/${crew.length} done</span></div>${crew
    .map((c, ci) => {
      const done = (c.left ?? 0) === 0

      return `<div class="crew-row${
        c.you ? " me" : ""
      }"><span class="crew-avatar">${esc((c.name || "?")[0].toUpperCase())}</span><div class="crew-meta"><div class="crew-line"><strong>${esc(c.name)}${
        c.you ? " · you" : ""
      }</strong><span class="tag ${
        done ? "" : "live"
      }" data-crew-tag="${sp.id}:${ci}">${
        done
          ? sicon("check") + " done"
          : `${c.left} sprint${c.left === 1 ? "" : "s"} left`
      }</span></div><div class="crew-track"><span class="crew-fill${
        c.live && !done ? " live" : ""
      }" data-crew-fill="${sp.id}:${ci}" style="width:${Math.min(100, Math.max(2, c.pct || 0))}%"></span></div></div>${
        c.live && !done
          ? '<span class="crew-dot" title="Focusing now"></span>'
          : ""
      }</div>`
    })
    .join("")}</div>`
...
function inviteInboxMarkup() {
  ...
  const decided = state.sprintInvites
...
function sprintBoardMarkup() {
  ...
  const groups = allGroups().filter((g) => get("sf-joined", []).includes(g.id))

  const friends = state.friends || []

  return `${inviteInboxMarkup()}<div class="card sprint-pro" style="margin-bottom:18px"><div class="section-row"><h2>${sicon("bolt")} Sprint rooms</h2><span class="tag">together</span></div>${
    active.length
      ? active
          .map((sp) => {
            ...
            const pend = (sp.invites || []).filter(
              (i) => i.status === "pending",
            ).length

            const acc = (sp.invites || []).filter(
              (i) => i.status === "accepted",
            ).length

            const dec = (sp.invites || []).filter(
              (i) => i.status === "declined",
            ).length

            const inviteLine =
              sp.visibility === "friends"
                ? `<span class="tag lock">${sicon("lock")} friends-only${
                    pend ? ` · ${pend} pending` : ""
                  }${acc ? ` · ${acc} in` : ""}${
                    dec ? ` · ${dec} out` : ""
                  }</span>`
                : `<span class="tag">${sicon("globe")} open room${
                    (sp.roster || []).length
                      ? ` · ${(sp.roster || []).length + 1} racing`
                      : ""
                  }</span>`

            return `<div class="sprint-room pro${
              live ? " live" : ""
            }"><div class="sprint-beam"></div><div class="section-row"><strong>${esc(sp.title)}</strong><span style="display:flex;gap:6px;flex-wrap:wrap"><span class="tag ${
              live ? "live" : ""
            }" data-sprint-cd="${sp.id}">${
              live ? "● LIVE" : "scheduled"
            }</span>${inviteLine}</span></div><p class="muted">${esc(sprintGroupName(sp))} · ${Math.round(sp.durationSec / 60)} min${
              sp.joined ? " · you're in" : ""
            }</p>${
              sp.purpose ? `<p class="purpose">“${esc(sp.purpose)}”</p>` : ""
            }${
              sp.joined || sp.visibility === "open"
                ? crewPaceMarkup(sp)
                : '<p class="muted">Join to see the crew pace board.</p>'
            }${
              (sp.invites || []).length
                ? `<div class="invite-chips">${sp.invites.map((i) => `<span class="invite-chip ${i.status}">@${esc(i.username)} · ${i.status === "accepted" ? "joined " + sicon("check") : i.status === "declined" ? "passed" : "invited…"}</span>`).join("")}</div>`
                : ""
            }<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">${
              sp.joined
                ? `${
                    live && !inSession
                      ? `<button type="button" class="primary" data-sprint-late="${sp.id}">Join late</button><button type="button" class="ghost" data-sprint-leave="${sp.id}">Leave</button>`
                      : ""
                  }${
                    live && inSession
                      ? '<span class="tag live">you\'re focusing ' +
                        sicon("fire") +
                        "</span>"
                      : ""
                  }${
                    !live
                      ? `<button type="button" class="primary" data-sprint-now="${sp.id}">Start now</button><button type="button" class="ghost" data-sprint-leave="${sp.id}">Leave</button>`
                      : ""
                  }`
                : `<button type="button" class="primary" data-sprint-join="${sp.id}">Join sprint</button>`
            }${
              owner
                ? `<button type="button" class="ghost" data-sprint-edit="${sp.id}">Rename / purpose</button><button type="button" class="ghost" data-sprint-nudge="${sp.id}" title="Simulate a friend replying right now">Nudge replies</button><button type="button" class="delete" data-sprint-del="${sp.id}" title="Delete room">Delete</button>`
                : ""
            }</div></div>`
          })
          .join("")
      : '<p class="muted">No live rooms. Start one below — shared suffering bonds people.</p>'
  }${
    past.length
      ? `<div class="section-row" style="margin-top:14px"><h3>Finish board</h3></div>${past.map((sp) => `<div class="board-row"><span>${esc(sp.title)}</span><span class="tag">${sp.result?.status === "done" ? `${sicon("check")} ${sp.result.focusedMin}m focused` : sp.result?.status === "dnf" ? "DNF" : "missed"}</span></div>`).join("")}`
      : ""
  }<div class="section-row" style="margin-top:16px"><h3>New sprint</h3><span class="tag">pro</span></div><div class="grid two"><input class="input" id="sprint-title" placeholder="Sprint title, e.g. Morning grind"><select class="select" id="sprint-group"><option value="">No group</option>${groups.map((g) => `<option value="${g.id}">${esc(g.name)}</option>`).join("")}</select><select class="select" id="sprint-dur"><option value="15">15 minutes</option><option value="25" selected>25 minutes</option><option value="30">30 minutes</option><option value="50">50 minutes</option></select><select class="select" id="sprint-in"><option value="2">Starts in 2 min</option><option value="5" selected>Starts in 5 min</option><option value="10">Starts in 10 min</option><option value="15">Starts in 15 min</option><option value="30">Starts in 30 min</option></select></div><textarea class="textarea autogrow" id="sprint-purpose" rows="2" placeholder="Purpose — why does this room exist? (shown to everyone)" style="margin-top:10px"></textarea><div style="margin-top:10px;position:relative"><select class="select" id="sprint-vis" style="width:100%" aria-label="Room visibility"><option value="open">Open — anyone can join</option><option value="friends">Friends — only people I pick</option></select><span class="fp-chip" id="sprint-picked" hidden></span><button type="button" class="ghost" id="sprint-friends-go" type="button" style="margin-top:8px;width:100%">👥 Choose friends — pick your crew</button></div><div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap"><button type="button" class="primary" id="sprint-create">Create sprint room</button></div></div>`
}
...
function challengeRow(c) {
  ...
  const pct = Math.min(
    100,
    Math.round((c.progress / Math.max(1, c.target)) * 100),
  )
  ...
  const board = [...(c.members || [])].sort(
    (a, b) =>
      challengeScore(c, b) - challengeScore(c, a) ||
      (b.minutes || 0) - (a.minutes || 0),
  )

  const top = Math.max(
    1,
    challengeScore(c, board[0] || { sessions: 0, minutes: 0 }),
  )

  const locked = state.activeChallengeId === c.id && !c.done

  const otherLock =
    !locked && !c.done && !c.joined && otherActiveChallenge(c.id)

  const medals = ["👑", "🥈", "🥉"]

  return `<div class="challenge pro"><div class="section-row"><strong>${esc(c.title)}</strong><span style="display:flex;gap:6px;flex-wrap:wrap"><span class="tag pace-tag">⚡ ${fmtPace(c)}/session</span>${
    c.done
      ? `<span class="tag">done ${sicon("party")}</span>`
      : locked
        ? `<span class="tag live">🔒 locked in</span>`
        : `<span class="tag">${c.progress}/${c.target} ${c.unit}</span>`
  }</span></div><div class="complete-track"><span class="complete-fill" style="width:${pct}%"></span></div><small class="muted">${esc(g?.name || "Open challenge")}${
    c.ownerId && c.ownerId === chatKey() ? " · you host" : ""
  }</small>${
    c.done && c.winner
      ? `<p class="crown-line">👑 <b>@${esc(c.winner)}</b> takes the crown</p>`
      : ""
  }<div class="board-lead"><div class="crew-head"><span>${sicon("trophy")} Leaderboard</span><span class="muted">${board.length} racing</span></div>${board
    .slice(0, 5)
    .map((m, i) => {
      const sc = challengeScore(c, m)

      return `<div class="lead-row${i === 0 ? " top" : ""}${
        m.you ? " me" : ""
      }"><span class="lead-rank">${medals[i] || `#${i + 1}`}</span><span class="crew-avatar sm">${esc((m.name || "?")[0].toUpperCase())}</span><div class="crew-meta"><div class="crew-line"><strong>${esc(m.name)}${
        m.you ? " · you" : ""
      }</strong><span class="muted">${m.sessions || 0} sess · ${m.minutes || 0}m</span></div><div class="crew-track"><span class="crew-fill${
        i === 0 && !c.done ? " live" : ""
      }" style="width:${Math.min(100, Math.max(3, Math.round((sc / top) * 100)))}%"></span></div></div></div>`
    })
    .join("")}</div>${
    !c.done
      ? `<div class="ch-actions">${
          c.joined
            ? `<button type="button" class="ghost" data-ch-leave="${c.id}">Leave challenge</button>`
            : `<button type="button" class="primary" data-ch-join="${c.id}"${
                otherLock
                  ? ' disabled title="Finish or leave your current challenge first"'
                  : ""
              } style="padding:8px 14px;font-size:12px">Join · locks timer to ${fmtPace(c)}</button>`
        }${
          !c.ownerId || c.ownerId === chatKey()
            ? `<button type="button" class="delete" data-ch-del="${c.id}" title="Delete challenge">Delete</button>`
            : ""
        }</div>`
      : `${
          !c.ownerId || c.ownerId === chatKey()
            ? `<div class="ch-actions"><button type="button" class="ghost" data-ch-del="${c.id}">Clear from board</button></div>`
            : ""
        }`
  }</div>`
}
...
function challengeMarkup() {
  ...
  const pendingCh = (state.challengeInvites || []).filter(
    (i) => i.status === "pending",
  )

  const decidedCh = (state.challengeInvites || [])
    .filter((i) => i.status !== "pending")
    .slice(-3)
    .reverse()
  ...
  const inbox =
    pendingCh.length || decidedCh.length
...
function eventMarkup() {
  ...
  return `${eventInboxMarkup()}<div class="card" style="margin-bottom:18px"><div class="section-row"><h2>Scheduled sessions</h2><span class="tag">commit</span></div>${
    upcoming.length
      ? upcoming
          .map((e) => {
            ...
            const pend = (e.invites || []).filter(
              (i) => i.status === "pending",
            ).length

            const going =
              (e.invites || []).filter((i) => i.status === "accepted").length +
              (e.mine ? 1 : 0)

            return `<div class="event-row pro"><div><strong>${sicon("calendar")} ${esc(e.title)}</strong><br><small class="muted">${eventWhen(e)} · ${e.durationMin} min${
              g ? ` · ${esc(g.name)}` : ""
            }${
              e.visibility === "friends" ? ` · ${sicon("lock")} friends` : ""
            } · ${going} going</small>${
              (e.invites || []).length
                ? `<div class="invite-chips">${e.invites.map((i) => `<span class="invite-chip ${i.status}">@${esc(i.username)} · ${i.status === "accepted" ? "in " + sicon("check") : i.status === "declined" ? "out" : "invited…"}</span>`).join("")}</div>`
                : ""
            }</div><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">${
              live
                ? `<span class="tag">● LIVE</span><button type="button" class="primary" data-event-start="${e.id}" style="padding:8px 12px;font-size:12px">Start</button>`
                : ""
            }<button type="button" class="${
              e.mine ? "ghost" : "primary"
            }" data-event-rsvp="${e.id}" style="padding:8px 12px;font-size:12px">${
              e.mine ? "Going " + sicon("check") : "RSVP"
            }</button>${
              owner
                ? `${
                    pend
                      ? `<button type="button" class="ghost" data-event-nudge="${e.id}" style="padding:8px 12px;font-size:12px" title="Simulate a friend replying now">Nudge</button>`
                      : ""
                  }<button type="button" class="delete" data-event-del="${e.id}" title="Remove">×</button>`
                : ""
            }</div></div>`
          })
          .join("")
      : '<p class="muted">Nothing scheduled. Put study on the calendar and show up.</p>'
...
function ensureEventFields() {
  ...
  const fresh = state.events.filter(
    (e) => e && e.at + (e.durationMin || 25) * 60000 > now - 7 * 86400000,
  )
...
function eventInboxMarkup() {
  const pending = (state.eventInvites || []).filter(
    (i) => i.status === "pending",
  )

  const decided = (state.eventInvites || [])
    .filter((i) => i.status !== "pending")
    .slice(-3)
    .reverse()
...
function resolveOneEventInvite(e, force) {
  ...
  if (pend.status === "accepted")
    addNotification(
      ...
      `@${pend.username} is going to “${e.title}”.`,
      "calendar",
    )
...
function resolveOneInvite(sp, force) {
  ...
  if (accept) {
    ...
    if (!crew.some((c) => c.id === pend.friendId))
      crew.push({
        id: pend.friendId || "peer-" + uid(),
        name: pend.username || "Friend",
        total: 4,
        left: 4,
        pct: 6,
        live: false,
    ...
    addNotification(
      "Sprint invite accepted",
      `@${pend.username} joined “${sp.title}”.`,
      "bolt",
    )
  }
...
function inviteRecipients(ids) {
  return (ids || []).filter(
    (id) => signedIn() && isUuid(id) && id !== state.user?.id,
  )
}
...
async function sendInviteDms(prefix, inv, toIds) {
  ...
  const titles = {
    sprint: "Sprint invite",
    session: "Session invite",
...
async function pushInviteDm(payload) {
  ...
  state.messages[payload.recipient_id] = [
    ...(state.messages[payload.recipient_id] || []),
    local,
  ]
...
function adoptIncomingInvite(inv, fromHandle, fromId) {
  ...
  if (
    state[store].some(
      (i) =>
        i.refId === inv.id || (i.sprintId === inv.id && inv.t === "sprint"),
    )
  )
    return false
  ...
  addNotification(
    label,
    `${fromHandle || "A friend"} invited you to “${inv.title}”`,
    inv.t === "session"
      ? "calendar"
      : inv.t === "challenge"
        ? "trophy"
  ...
  try {
    browserNotify(
      label,
      `Invited to “${inv.title}” — open Community to respond.`,
    )
  } catch {
...
function adoptInviteReply(reply, from) {
  ...
  if (reply.t === "session") {
    ...
    const pend = (ev.invites || []).find(
      (i) => i.status === "pending" && i.username === who,
    )
  ...
  const pend = (sp.invites || []).find(
    (i) =>
      i.status === "pending" && (i.friendId === from?.id || i.username === who),
  )
...
function harvestInvitesFromRows(rows, resolveFrom) {
  ...
  for (const r of rows || []) {
    ...
    if (meta.invite && adoptIncomingInvite(meta.invite, from?.handle, from?.id))
      changed = true

    if (meta.inviteReply && adoptInviteReply(meta.inviteReply, from))
      changed = true
  }
...
function refreshInviteSurfaces(root) {
  ...
  if (
    root &&
    (root.id === "tab-timer" || root.querySelector?.(".mission-desk"))
  ) {
    ...
    try {
      ...
      /* shell always available after boot */
    }
...
function acceptChallengeInvite(invId, root) {
  ...
  const existing = (state.challenges || []).find(
    (c) => c.id === inv.refId || c.title === inv.title,
  )
...
function missionDeskMarkup() {
  ...
  const pendingInvites = [
    ...
    ...(state.eventInvites || [])
      .filter((i) => i.status === "pending")
      .map((i) => ({ ...i, kind: "session" })),

    ...(state.challengeInvites || [])
      .filter((i) => i.status === "pending")
      .map((i) => ({ ...i, kind: "challenge" })),
  ].slice(0, 3)

  const inviteBlock = pendingInvites.length
    ? `<div class="mission-invites">${pendingInvites
        .map((inv) => {
          const icon =
            inv.kind === "challenge"
              ? "trophy"
          ...
          const when =
            inv.kind === "challenge"
              ...
              : inv.kind === "session"
                ? new Date(inv.at || now).toLocaleString([], {
                    weekday: "short",
                    hour: "2-digit",
                    minute: "2-digit",
                  })
          ...
          const acc =
            inv.kind === "challenge"
              ? `data-ch-inv-accept="${inv.id}"`
              : inv.kind === "session"
                ? `data-ev-inv-accept="${inv.id}"`
                : `data-inv-accept="${inv.id}"`

          const dec =
            inv.kind === "challenge"
              ? `data-ch-inv-decline="${inv.id}"`
              : inv.kind === "session"
                ? `data-ev-inv-decline="${inv.id}"`
                : `data-inv-decline="${inv.id}"`
  ...
  const live = (state.sprints || []).find(
    (s) =>
      s.joined &&
      !s.result &&
      now >= s.startsAt &&
      now < s.startsAt + s.durationSec * 1000,
  )
  ...
  const next = (state.sprints || [])
    ...
    .filter(
      (s) =>
        s.joined &&
        !s.result &&
        s.startsAt + s.durationSec * 1000 > now &&
        (!live || s.id !== live.id),
    )
  ...
  if (!focus && !challenges.length && !upcomingEv) {
    if (inviteBlock)
      return `<div class="card mission-desk idle"><div class="mission-top"><span class="mission-eyebrow">${sicon("gift")} Pending invites</span><button type="button" class="ghost" data-mission-goto>Open Sprints</button></div>${inviteBlock}<p class="muted">Accept an invite and it docks here as a live mission.</p></div>`
  ...
  const inSession =
    focus &&
    (state.sprintSession === focus.id ||
      (state.running && state.mode === "focus"))

  return `${inviteBlock}<div class="card mission-desk${
    live ? " live" : ""
  }"><div class="mission-glow"></div><div class="mission-top"><span class="mission-eyebrow"><span class="mission-dot"></span> Live mission · Focus desk</span><button type="button" class="ghost" data-mission-goto>Open Sprints</button></div>${
    focus
      ? `<div class="mission-sprint"><div><strong>${esc(focus.title)}</strong><br><small class="muted">${
          live
            ? "happening now"
            : "starts " +
        ...
        } · ${Math.round(focus.durationSec / 60)} min${
          focus.purpose
            ? ` · ${esc(focus.purpose.slice(0, 80))}${
                focus.purpose.length > 80 ? "…" : ""
              }`
            : ""
        }</small></div><span class="tag ${
          live ? "live" : ""
        }" data-mission-cd="${focus.id}">${
          live ? "● LIVE" : "scheduled"
        }</span></div>${
          focus && live && !inSession
            ? `<button type="button" class="primary" data-mission-join="${focus.id}">Jump in — start focus</button>`
            : ""
        }${
          focus && live && inSession
            ? `<span class="tag live">${sicon("fire")} you're racing this one</span>`
            : ""
        }`
      : ""
  }${challenges
    .map((c) => {
      ...
      return `<div class="mission-row"><span>${sicon("trophy")} ${
        locked ? "🔒 " : ""
      }${esc(c.title)}</span><span class="tag">${c.progress}/${c.target}</span></div><div class="crew-track slim"><span class="crew-fill" style="width:${pct}%"></span></div>`
    })
    .join("")}${
    upcomingEv
      ? `<div class="mission-row"><span>${sicon("calendar")} ${esc(upcomingEv.title)} · ${new Date(upcomingEv.at).toLocaleString([], { weekday: "short", hour: "2-digit", minute: "2-digit" })}</span>${
          Date.now() >= upcomingEv.at
            ? `<button type="button" class="primary" data-mission-event="${upcomingEv.id}" style="padding:7px 12px;font-size:12px">Start</button>`
...
function bindMissionDesk(root) {
  ...
  $$("[data-inv-accept]", root).forEach(
    (b) => (b.onclick = () => acceptSprintInvite(b.dataset.invAccept, root)),
  )

  $$("[data-inv-decline]", root).forEach(
    (b) => (b.onclick = () => declineSprintInvite(b.dataset.invDecline, root)),
  )

  $$("[data-ev-inv-accept]", root).forEach(
    (b) => (b.onclick = () => acceptEventInvite(b.dataset.evInvAccept, root)),
  )

  $$("[data-ev-inv-decline]", root).forEach(
    (b) => (b.onclick = () => declineEventInvite(b.dataset.evInvDecline, root)),
  )

  $$("[data-ch-inv-accept]", root).forEach(
    (b) =>
      (b.onclick = () => acceptChallengeInvite(b.dataset.chInvAccept, root)),
  )

  $$("[data-ch-inv-decline]", root).forEach(
    (b) =>
      (b.onclick = () => declineChallengeInvite(b.dataset.chInvDecline, root)),
  )
  ...
  $$("[data-mission-join]", root).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (mjLock)
          return notify(
            `Challenge lock holds your timer — leave “${mjLock.title}” first`,
          )
  ...
  $$("[data-mission-event]", root).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (meLock)
          return notify(
            `Challenge lock holds your timer — leave “${meLock.title}” first`,
          )
...
function bindSprints(body) {
  ...
  $("#sprint-create", body).onclick = () => {
    ...
    const dur = Math.min(
      120,
      Math.max(5, parseInt($("#sprint-dur", body).value, 10) || 25),
    )
    ...
    const visibility =
      $("#sprint-vis", body)?.value === "friends" ? "friends" : "open"

    const picked = [
      ...new Set([...sprintPickedIds, ...cloudFriends.map((f) => f.id)]),
    ].filter((id) => sprintPickedIds.includes(id))
    ...
    if (groupId) {
      postGroupMessage(
        groupId,
        `Sprint room open: “${title}”${
          purpose ? ` — ${purpose}` : ""
        } — ${dur} min, starts in ${mins} min. Join from Community → Sprints.`,
        "bolt",
      )
    ...
    sendInviteDms(
      ...
      invites.map((i) => i.friendId),
    ).catch(() => {})

    if (visibility === "friends")
      addNotification(
        "Sprint invites sent",
        `“${title}” — ${invites.map((i) => "@" + i.username).join(", ")}. They can join or pass.`,
        "gift",
      )
    ...
    notify(
      visibility === "friends"
        ? `Room created — invites sent to ${invites.length} friend${
            invites.length === 1 ? "" : "s"
          }`
        : "Sprint room created — see you at the start",
    )
  }
  ...
  $$("[data-inv-accept]", body).forEach(
    (b) => (b.onclick = () => acceptSprintInvite(b.dataset.invAccept, body)),
  )

  $$("[data-inv-decline]", body).forEach(
    (b) => (b.onclick = () => declineSprintInvite(b.dataset.invDecline, body)),
  )

  $$("[data-ch-inv-accept]", body).forEach(
    (b) =>
      (b.onclick = () => acceptChallengeInvite(b.dataset.chInvAccept, body)),
  )

  $$("[data-ch-inv-decline]", body).forEach(
    (b) =>
      (b.onclick = () => declineChallengeInvite(b.dataset.chInvDecline, body)),
  )
  ...
  $$("[data-sprint-del]", body).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (!isSprintOwner(sp))
          return notify("Only the room creator can delete it")
        ...
        confirmBox(
          ...
          {
            eyebrow: "Sprint rooms",
            yesLabel: "Delete room",
            noLabel: "Keep it",
          },
        )
      }),
  ...
  $$("[data-sprint-late]", body).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (lateLock)
          return notify(
            `Challenge lock holds your timer — leave “${lateLock.title}” first`,
          )
  ...
  $("#ch-create", body).onclick = () => {
    ...
    const target = Math.min(
      500,
      Math.max(1, parseInt($("#ch-target", body).value, 10) || 10),
    )

    const groupId = $("#ch-group", body).value || ""

    if (!groupId && !challengePickedIds.length)
      return notify("Join a group or pick friends to challenge")

    const unit =
      $("#ch-unit", body).value === "minutes" ? "minutes" : "sessions"

    const title =
      $("#ch-title", body).value.trim() || `${target} ${unit} this week`

    let paceMin = Math.min(
      180,
      Math.max(0, parseInt($("#ch-min", body)?.value, 10)),
    )

    let paceSec = Math.min(
      59,
      Math.max(0, parseInt($("#ch-sec", body)?.value, 10)),
    )
    ...
    if (paceMin === 0 && paceSec === 0)
      return notify("Pace can't be 0:00 — set at least a few seconds")
    ...
    postGroupMessage(
      groupId,
      `New challenge: “${title}” — ${target} ${unit} this week at ${fmtPace(ch)} a session. Who's in?`,
      "trophy",
    )
    ...
    notify(
      seated
        ? `Challenge launched — your timer is locked to ${fmtPace(ch)}`
        : "Challenge launched — leave your current race to join it",
    )
  }
    ...
    (b) =>
      (b.onclick = () => {
        if (joinChallenge(b.dataset.chJoin)) {
          ...
          notify(
            `Locked in — every session runs ${
              c ? fmtPace(c) : "the set pace"
            } ${sicon("bolt")}`,
          )
        }
      }),
  ...
  $$("[data-ch-del]", body).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (c.ownerId && c.ownerId !== chatKey())
          return notify("Only the host can delete this challenge")
...

  {
    ...
    if (groupSel)
      groupSel.hidden = ($("#ev-aud", body)?.value || "self") !== "group"
  }
  ...
  $("#ev-create", body).onclick = () => {
    ...
    if (aud === "group" && !groupId)
      return notify("Pick a group — or switch to Just me / Friends")

    const picked = [...eventPickedIds]

    if (aud === "friends" && !picked.length)
      return notify("Friends session? Choose at least one friend first")
    ...
    if (e.groupId) {
      postGroupMessage(
        e.groupId,
        `Scheduled: “${title}” — ${new Date(at).toLocaleString([], { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}. RSVP in Community → Sprints.`,
        "calendar",
      )
    ...
    if (e.visibility === "friends")
      addNotification(
        ...
        `“${title}” — ${invites.map((i) => "@" + i.username).join(", ")}. They can RSVP or pass.`,
        "gift",
      )
    ...
    notify(
      e.visibility === "friends"
        ? `Scheduled — invites sent to ${invites.length} friend${
            invites.length === 1 ? "" : "s"
          }`
        : "Session scheduled — I'll remind you",
    )
  }

  $$("[data-ev-inv-accept]", body).forEach(
    (b) => (b.onclick = () => acceptEventInvite(b.dataset.evInvAccept, body)),
  )

  $$("[data-ev-inv-decline]", body).forEach(
    (b) => (b.onclick = () => declineEventInvite(b.dataset.evInvDecline, body)),
  )
  ...
  $$("[data-event-del]", body).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (!isEventOwner(e))
          return notify("Only the host can remove this session")
  ...
  $$("[data-event-start]", body).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        if (evLock)
          return notify(
            `Challenge lock holds your timer — leave “${evLock.title}” first`,
          )
        ...
        notify(
          `${sicon("check")} “${esc(e.title)}” started — show up for yourself`,
        )
      }),
  )
}
...
function pruneExpiredStories() {
  ...
  if (state.statusSeen) {
    ...
    Object.keys(state.statusSeen).forEach((k) => {
      if (k.startsWith("story:") && !liveIds.has(k)) delete state.statusSeen[k]
    })
  }
...
function deleteStatus(id) {
  ...
  if (!target) return { ok: false, error: "That status no longer exists." }
...
// Cloud story media staging: file + one object URL, revoked on clear, post,

// replace, or leaving the editor. Never persisted, never in the database.

// Replaced by the multi-step story editor state below (kept as the single

// staging slot the old composer API expected).
...
// Preview-time audio (music picker demo + attached clip audition). One element.

let storyPreviewAudio = null

// songId → storage path for already-uploaded library tracks (avoids re-upload).
...
function stopStoryPreviewAudio() {
  if (storyPreviewAudio) {
    try {
      storyPreviewAudio.pause()
    } catch {
      /* ignore */
    }

    try {
      storyPreviewAudio.src = ""
    } catch {
      /* ignore */
    }

    storyPreviewAudio = null
  }
}

function clearStoryEditorMedia() {
  try {
    if (storyEditor.fileUrl) URL.revokeObjectURL(storyEditor.fileUrl)
  } catch {
    /* ignore */
  }

  storyEditor.file = null
...
// Music object URL for the current editor clip (local library bytes only).

async function storyMusicObjectUrl(songId) {
    ...
    .map(
      (s) =>
        `<button type="button" class="story-ring${
          isSeen("story:" + s.id) ? " seen" : ""
        }" data-story-view="${s.id}" title="${
          ownStory(s.id)
            ? "View in status loop — right-click to delete"
            : "View in status loop"
        }"><span>${
          s.photo
            ? `<img src="${s.photo}" alt="Story">`
            : s.kind === "video"
              ? sicon("film")
              : sicon("fire")
        }</span><small>${esc(s.author.length > 8 ? s.author.slice(0, 7) + "…" : s.author)}</small></button>`,
    )
    .join("")
}
...
function storiesMarkup() {
  ...
  const unseenCloud = cloudStories.filter(
    (s) => !isSeen("cstory:" + s.id),
  ).length

  return `<div class="card" style="margin-bottom:18px"><div class="section-row"><h2>Stories</h2><span class="tag">24h${
    unseen + unseenCloud ? ` · ${unseen + unseenCloud} new` : ""
  }</span></div><div class="story-strip"><button type="button" class="story-add" data-status-play title="Play the full status loop">${sicon("play")}<small>Status</small></button>${localStoryRings(live)}${cloudStoryGroups()
    .filter((g) => cloudFriends.some((f) => f.id === g.author.id))
    .map((g) =>
      friendStatusRing(g.author.id, g.author.handle, g.author.name, g.items),
    )
    .join(
      "",
    )}</div><p class="muted" style="margin:8px 0 0">Statuses are view-only here — post from Community → Status.</p></div>`
}

function cloudStoryCaption(s) {
  return String(s?.text || "").trim()
}

function cloudStoryMusic(s) {
...
function storyTextBgClass(s) {
  const bg = s?.meta?.bgStyle || s?.bgStyle || "aurora"

  return STORY_TEXT_BGS.includes(bg) ? bg : "aurora"
}
...
function storyTextAlignStyle(s) {
  const al = s?.meta?.textAlign || s?.textAlign || "center"

  return ["left", "center", "right"].includes(al) ? al : "center"
}

function storyEmojiOf(s) {
  return String(s?.meta?.emoji || s?.emoji || "").slice(0, 8)
}
...
// draft, an attached file stays staged, and the user is told plainly.

async function postCloudStory({
  text,
  vis = "connections",
  file = null,
  btn = null,
  ...
  const storyMeta = meta || buildStoryMeta(ed)

  // Photo/video need a file; text needs words (music alone isn't a status).

  if ((mediaKind === "image" || mediaKind === "video") && !file) {
    notify(
      mediaKind === "video" ? "Choose a video first" : "Choose a photo first",
    )

    return false
  }

  if (mediaKind === "text" && !clean) {
  ...
  if (!navigator.onLine) {
    ...
    notify(
      file
        ? "You're offline — media stories need a connection. Your file is still attached for when you're back."
  ...
  const trace = { at: new Date().toISOString(), kind: mediaKind, visibility }
  ...
  const traceLog = (stage, extra = {}) => {
    try {
      ...
      /* diagnostics must never break posting */
    }
  }
  ...
  try {
    ...
    if (file) {
      ...
      ed.busyLabel = isVideo ? "Uploading video…" : "Uploading photo…"
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ;({
          ...
          ? await uploadStoryVideo(file)
          : await uploadStoryPhoto(file))

        if (!upErr || !isTransientUploadError(upErr)) break

        traceLog("storage-retry", {
          attempt: attempt + 1,
          error: String(upErr.message || upErr).slice(0, 160),
        })
    ...
    const { data: row, error } = await createStory({
      text: clean,
      mediaPath,
      kind: kindOut,
      visibility,
    ...
    notify(`${kindLabel} shared for 24h${storyMeta.music ? " with music" : ""}`)

    // Fresh editor for the next status (keep visibility preference).

    const keepVis = visibility
    ...
    // Re-render so the editor returns to an interactive state.
...
function bindStories(body) {
  if (signedIn()) {
    ...
    refreshCloudStories(false)
      .then(() => {
        if (
          state.tab === "community" &&
          state.subtab === "discover" &&
          JSON.stringify(cloudStories.map((s) => s.id)) !== before
        )
          renderCommunity()
      })
      .catch(() => {})
  ...
  $$("[data-cloud-story-group]", body).forEach(
    (b) =>
      (b.onclick = () => {
        const g = cloudStoryGroups().find(
          (x) => x.author.id === b.dataset.cloudStoryGroup,
        )
...
async function refreshCloudStories(force) {
  ...
  try {
    ...
    const { data: profiles } = await getPublicProfiles(
...
function renderStatus(body) {
  ...
  const myRows = mine
    .map((s) => {
      ...
      const preview =
        (s.kind === "image" || s.kind === "video") && s.mediaPath
          ? `<img class="sv-list-thumb" data-story-thumb="${esc(s.mediaPath)}" alt="">`
          : `<span class="sv-list-thumb sv-list-thumb-text">${sicon(s.kind === "video" ? "film" : s.kind === "image" ? "camera" : "fire")}</span>`

      const musicTag = cloudStoryMusic(s) ? ` ${sicon("music")}` : ""

      return (
        `<button type="button" class="status-row" data-mine-status="${esc(s.id)}">` +
        ...
        preview +
        `<span class="status-meta"><strong>${esc(String(s.text || (s.kind === "video" ? "Video status" : s.kind === "image" ? "Photo status" : "Status")).slice(0, 48))}${musicTag}</strong><small>${relTime(s.createdAt)}${
          s.visibility === "private" ? " · Only me" : ""
        }${s.kind === "video" ? " · video" : ""}</small></span>` +
        (unseen
          ? '<span class="status-dot" aria-label="Unseen"></span>'
          : `<span class="tag">${sicon("check")}</span>`) +
        `</button>`
      )
    })
    .join("")
  ...
  body.innerHTML =
    ...
    (signedIn()
      ? `<div class="card" style="margin-bottom:18px"><div class="section-row"><h2>Friends' statuses</h2><span class="tag">${
          ...
          friendGroups.length
            ? `<div class="story-strip">${friendRings}</div>`
            : `<p class="muted">No friend statuses right now — you'll see rings here the moment someone posts.</p>`
        }</div>`
      : "") +
    (device.length
      ? `<div class="card"><div class="section-row"><h2>On this device</h2><span class="tag">local</span></div><div class="story-strip">${localStoryRings(device)}</div></div>`
      : "")
...
function storyMusicRowMarkup() {
  ...
  const span = Math.max(1, Math.round((m.end || 0) - (m.start || 0)))
...
function bindStatusHome(body) {
  ...
  $$("[data-st-kind]", body).forEach((b) => {
    b.onclick = () => {
      if (storyEditor.busy) return

      const kind = b.dataset.stKind
      ...
      Object.assign(storyEditor, keep)
  ...
  file?.addEventListener("change", async () => {
    ...
    const isVideo =
      storyEditor.kind === "video" ||
      String(picked.type || "").startsWith("video/")

    if (isVideo) {
      if (picked.size > 50 * 1024 * 1024)
        return notify("Video is too large. Maximum size is 50 MB.")
      ...
      if (
        ...
        return notify("Use an MP4, WebM or MOV video.")
    } else {
    ...
    storyEditor.file = picked

    storyEditor.fileUrl = URL.createObjectURL(picked)

    // Keep the legacy slot in sync for any external readers.
  ...
  $("[data-st-media-remove]", body)?.addEventListener("click", () => {
    clearStoryEditorMedia()
  ...
  $("#st-post", body)?.addEventListener("click", (e) => {
    ...
    postCloudStory({
      ...
      kind:
        storyEditor.kind === "video"
          ? "video"
          : storyEditor.kind === "photo"
            ? "image"
            : "text",

      meta: buildStoryMeta(storyEditor),
...
function closeStoryMusicPicker() {
  ...
  storyMusicPickerEl = null

  document.removeEventListener("keydown", storyMusicPickerKey)
}

function storyMusicPickerKey(e) {
...
function openStoryMusicPicker() {
  ...
  ov.innerHTML = `<div class="st-music-modal" role="dialog" aria-label="Add music">
    ...
    <div class="st-music-foot">
      ...
      <button type="button" class="primary" data-smp-attach disabled>Attach</button>
    </div>
  </div>`

  document.body.append(ov)

  storyMusicPickerEl = ov

  document.addEventListener("keydown", storyMusicPickerKey)

  const list = ov.querySelector("[data-smp-list]")

  const clip = ov.querySelector("[data-smp-clip]")

  const attachBtn = ov.querySelector("[data-smp-attach]")
  ...
  const playPreview = async () => {
    ...
    storyPreviewAudio = audio

    audio.volume = cur.vol ?? 0.7
...
function musicPickRow(s, on) {
  return `<button type="button" class="st-music-row-item${
    on ? " on" : ""
  }" role="option" aria-selected="${
    on ? "true" : "false"
  }" data-smp-song="${esc(s.id)}">
    <span class="st-music-ico">${sicon("music")}</span>
    <span class="st-music-meta"><strong>${esc(s.title || "Untitled")}</strong><small>${esc(s.artist || s.fileName || "")}${
      s.duration ? ` · ${fmtClock(s.duration)}` : ""
    }</small></span>
    ${on ? `<span class="tag">${sicon("check")}</span>` : ""}
  </button>`
}
...
function openStoryMenu(anchorBtn, onDelete) {
  ...
  menu.style.left =
    Math.max(m, Math.min(b.right - r.width, window.innerWidth - r.width - m)) +
    "px"

  menu.style.top =
    Math.max(m, Math.min(b.bottom + 6, window.innerHeight - r.height - m)) +
    "px"
...
function openStoryViewer(items, startIdx) {
  ...
  ov.innerHTML = `<div class="sv-frame" role="dialog" aria-label="Status viewer"><div class="sv-progress" data-sv-progress></div><div class="sv-top"><span class="status-avatar sm" data-sv-avatar></span><span class="sv-who"><strong data-sv-name></strong><small data-sv-time></small></span><button type="button" class="sv-more" data-sv-more aria-label="Status options" title="Status options" hidden>⋮</button><button type="button" class="sv-close" data-sv-close aria-label="Close status viewer">×</button></div><div class="sv-stage" data-sv-stage></div><button type="button" class="sv-tap left" data-sv-prev aria-label="Previous status">‹</button><button type="button" class="sv-tap right" data-sv-next aria-label="Next status">›</button><div class="sv-foot"><span class="muted" data-sv-views></span><span data-sv-music></span></div></div>`

  document.body.append(ov)

  // Attached music / video for the current item — torn down on every advance

  // and when the viewer closes (closeStoryViewer calls __svStopMedia).

  let mediaAudio = null

  let mediaVideo = null

  let mediaUrlOwned = false

  function stopItemMedia() {
    if (mediaAudio) {
      try {
        mediaAudio.pause()
      } catch {
    ...
    const pill = ov.querySelector("[data-sv-music]")
  ...
  function paintBars() {
    bar().innerHTML = st.items
      .map(
        (_, i) =>
          `<span class="sv-bar${
            i < st.idx ? " done" : i === st.idx ? " now" : ""
          }"><i></i></span>`,
      )
      .join("")
  }
  ...
  function pause() {
    ...
    if (st.timer) st.remaining = Math.max(0, st.endsAt - Date.now())
  ...
  function show(i) {
    ...
    clearTimer() // a video "ended" or tap must cancel the previous dwell
    ...
    ov.querySelector("[data-sv-avatar]").innerHTML = statusAvatarInner(
      item.handle,
      item.name,
      null,
    )

    ov.querySelector("[data-sv-avatar]").style.setProperty(
      "--sv-accent",
      svAccent(item.handle),
    )

    ov.querySelector("[data-sv-name]").textContent = item.mine
      ? "My Status"
      : "@" + item.handle

    ov.querySelector("[data-sv-time]").textContent =
      `${relTime(item.createdAt)} · ${item.visibility}`
    ...
    const capText = String(item.text || "").trim()
    ...
    const musicPill = ov.querySelector("[data-sv-music]")
    ...
    if (music?.path && signedIn()) {
      ...
      getStoryMediaUrl(music.path)
        .then(({ data, error } = {}) => {
          ...
          audio.volume = Math.max(0, Math.min(Number(music.vol ?? 0.7), 1))

          const start = Math.max(0, Number(music.start) || 0)

          const end = Math.max(start + 1, Number(music.end) || start + 15)
          ...
          if (
            ...
            const tgl = musicPill.querySelector("[data-sv-music-toggle]")
    ...
    if ((item.kind === "image" || item.kind === "video") && item.mediaPath) {
      ...
      if (isVideo) {
        ...
        const beginIfCurrent = (ms) => {
          if (st.gen === gen) begin(ms)
        }
        ...
        vid.addEventListener("loadedmetadata", () => {
          ...
          const d =
            Number.isFinite(vid.duration) && vid.duration > 0 ? vid.duration : 7

          beginIfCurrent(Math.min(Math.max(d * 1000, 3000), 60000))

          if (!st.paused) vid.play().catch(() => {})
        })
        ...
        getStoryMediaUrl(item.mediaPath)
          .then(({ data, error } = {}) => {
            if (!vid.isConnected || st.items[st.idx]?.id !== item.id) return
      ...
      // Text-only status: styled card (bg / align / size / style / emoji).

      const bg = storyTextBgClass({ meta })

      const sz = storyTextSizeClass({ meta })

      const tst = storyTextStyleClass({ meta })

      const al = storyTextAlignStyle({ meta })

      const em = storyEmojiOf({ meta })

      const body = (em ? `${em} ` : "") + (item.text || "")
    ...
    prevBtns.forEach((b) => {
      if (b) b.disabled = st.idx === 0
    })

    nextBtns.forEach((b) => {
      if (b) b.disabled = st.idx === st.items.length - 1
    })
  ...
  // Hold-to-pause on the stage; ignore presses on native video controls,

  // links, and the music pill so scrubbing/toggles stay usable.
  ...
  stage().addEventListener("pointerup", stageRelease)

  stage().addEventListener("pointercancel", stageRelease)
  ...
  if (item.type === "story" && item.kind === "photo") {
    ...
    v.addEventListener(
      ...
      () => {
        ...
        if (token === statusNavToken && statusSeq[statusIdx] === item) begin()
      },
...
function renderStatusProgress(overlay, frac) {
  ...
  bar.innerHTML = statusSeq
    ...
    .map(
      (_, i) =>
        `<span class="st-seg${
          i < statusIdx ? " done" : i === statusIdx ? " current" : ""
        }"><i data-st-fill="${i}" style="width:${
          i < statusIdx ? 100 : 0
        }%"></i></span>`,
    )
...
function surfacePillInfo(surface) {
  const s =
    surface === "groups"
      ? backendConfigured
        ? groupsBackendStatus || "probing"
        : "local"
      : surface === "friends"
        ? signedIn()
          ? friendsBackendStatus || "probing"
          : "local"
  ...
  const title =
    s === "ok"
      ? surface === "groups"
        ? "Group list is live from the backend"
        : "Connections are live from the backend"
      : s === "degraded"
        ? surface === "groups"
          ? "Groups load, but the server is partially degraded (member counts unavailable). Run migration 018 to fix."
          : "Connections load, but profile names couldn't be fetched — handles are shown instead."
        : s === "down"
          ? "The server responded with an error. Retrying automatically."
          : s === "offline"
            ? surface === "groups"
              ? "Can't reach the server — showing cached groups. Check your connection."
              : "Can't reach the server — showing cached connections. Check your connection."
            : s === "local"
              ? surface === "sprints"
                ? "Sprints and challenges live entirely on this device — nothing to fetch."
                : "Friends are stored on this device. Sign in to sync connections."
              : "Checking the backend…"

  return { cls, label, title, retryable: s !== "local" }
}

for (const s of ["groups", "friends", "sprints"])
  registerBackendPillResolver(s, surfacePillInfo)
...
function renderDiscover(body) {
  ...
  body
    .querySelector("[data-backend-retry]")
    ?.addEventListener("click", async () => {
      ...
      if (groupsBackendStatus === "ok" || groupsBackendStatus === "degraded") {
        ...
        if (grid) {
          ...
          grid.innerHTML =
            subjectGroups.map(groupCard).join("") ||
            '<p class="muted">No groups match this subject yet.</p>'
  ...
  body.insertAdjacentHTML(
    "beforeend",
    `<section class="feed-section"><div class="section-row"><div><div class="eyebrow">Community feed</div><h2 style="margin:5px 0 0">Study notes from the community</h2></div><span class="tag">${visiblePosts().length} posts</span></div><div class="card composer"><textarea class="textarea autogrow" id="post-composer" rows="2" placeholder="Share a useful study insight, milestone, or question..."></textarea><div class="composer-actions"><span class="muted">Be generous with what you learn.</span><button type="button" class="primary" id="publish-post">Publish note</button></div></div><div class="feed-list">${
      visiblePosts().length
        ? visiblePosts()
            .map(
              (post) =>
                `<article class="card post" data-post="${post.id}"><div class="post-author"><div class="avatar">${avatarMarkup(post.photo, post.avatar || state.profile.avatar)}</div><div><strong>${esc(post.author || state.profile.name)}</strong><small>@${esc(post.handle || state.profile.handle)}${
                  post.university ? ` · ${esc(post.university)}` : ""
                } · ${new Date(post.time).toLocaleDateString()}</small></div>${postMenuMarkup(post)}</div><div class="post-text" data-post-text="${post.id}"><p>${esc(post.text)}</p>${
                  post.editedAt ? '<small class="muted">(edited)</small>' : ""
                }</div><div class="post-actions"><button type="button" data-like-post="${post.id}" class="${
                  (post.likedBy || []).includes(postOwnerId()) ? "liked" : ""
                }">${
                  (post.likedBy || []).includes(postOwnerId())
                    ? sicon("heart-filled")
                    : sicon("heartOutline")
                } ${post.likes || 0}</button><button type="button" data-comment-toggle="${post.id}">${sicon("chat")} ${
                  (post.comments || []).length ? `${post.comments.length} ` : ""
                }Comments</button></div><div class="post-comments" data-comments="${post.id}" hidden></div></article>`,
            )
            .join("")
        : '<div class="card empty-state"><div class="emoji">' +
          sicon("sparkle") +
          '</div><h3>The feed is waiting for your first note</h3><p class="muted">Share a small insight and make someone else’s study session easier.</p></div>'
    }</div></section>`,
  )
...
function postMenuMarkup(post) {
  ...
  return `<div class="post-menu-wrap"><button type="button" class="icon-btn post-menu-btn" data-post-menu="${post.id}" title="Note options" aria-label="Note options">⋮</button><div class="post-menu" data-post-pop="${post.id}" hidden>${
    own
      ? `<button type="button" data-post-edit="${post.id}">Edit</button><button type="button" class="danger" data-post-del="${post.id}">Delete</button>`
      : `<button type="button" data-post-block="${esc(post.handle || post.author || "")}" data-post-name="${esc(post.author || post.handle || "this user")}">Block author</button>`
  }</div></div>`
}
...
function groupCard(g) {
  ...
  const vis =
    g.visibility === "private"
      ...
      : g.source === "cloud"
        ? `<span class="tag" title="Listed in Discover">public</span>`
        : ""

  const role =
    g.myRole && g.myRole !== "member"
      ? `<span class="tag">${esc(g.myRole)}</span>`
      : ""

  return `<article class="card group-card"><div class="group-top">${groupAvatarMarkup(g)}<div><h3>${esc(g.name)}</h3><span class="muted">${g.members || 1} learners</span> ${vis}${role}</div></div><p class="muted" style="margin-top:13px">${esc(g.description)}</p><div>${(g.tags || []).map((tag) => `<span class="tag" style="margin-right:4px">#${esc(tag)}</span>`).join("")}</div><div class="actions">${
    joined
      ? `<button type="button" class="primary" data-open-group="${g.id}" style="flex:1;padding:9px">Open chat</button><button type="button" class="ghost" data-leave-group="${g.id}">Leave</button>`
      : `<button type="button" class="ghost" data-join-group="${g.id}" style="flex:1">Join group</button>`
  }</div></article>`
}
    ...
    (b) =>
      (b.onclick = () => {
        ...
        if (signedIn()) {
          ...
          groupLeave(b.dataset.leaveGroup)
            .then(({ error } = {}) => {
...
function renderMyGroups(body) {
  ...
  $("#create-group", body).onclick = async () => {
    ...
    const tags = $("#new-group-focus")
      .value.split(",")
      .map((x) => x.trim())
      .filter(Boolean)

    const visibility =
      $("#new-group-vis")?.value === "public" ? "public" : "private"
    ...
    if (signedIn()) {
      ...
      try {
        const { data, error } = await groupCreate({
          name,
          description: desc,
          logo,
          topics: tags,
        ...
        notify(
          visibility === "public"
            ? "Group created — listed in Discover"
            : "Private group created — invite members from the chat",
        )

        renderCommunity()

        refreshCloudGroups(true)
          .then(() => {
        ...
        if (isPhase7Missing(err)) {
          notify(
            "Cloud groups need the latest database update — saved on this device instead",
          )
...
function redeemReferral(root) {
  ...
  if (!/^SF-[A-Z0-9]{6}$/.test(code)) return notify("Codes look like SF-AB12CD")
...
function bindCloudFriends(body) {
  ...
  $$("[data-fr-add]", body).forEach(
    (b) =>
      (b.onclick = async () => {
        ...
        notify(
          error
            ? "Couldn't send — " + String(error.message).slice(0, 90)
            : "Request sent",
        )
  ...
  $$("[data-fr-accept]", body).forEach(
    (b) =>
      (b.onclick = async () => {
        ...
        notify(
          error
            ? "Couldn't accept — " + String(error.message).slice(0, 90)
            : "Connected — they were notified",
        )
  ...
  $$("[data-fr-unfriend]", body).forEach(
    (b) =>
      (b.onclick = () => {
        confirmBox(
          "Remove this connection?",
          "You will stop seeing each other's updates and DMs will close.",
          async () => {
  ...
  $$("[data-fs-add]", pop).forEach(
    (b) =>
      (b.onclick = async () => {
        ...
        notify(
          error
            ? "Couldn't send request — " + String(error.message).slice(0, 90)
  ...
  input.addEventListener("input", () => {
    ...
    if (q.length < 2) {
      friendNameSearch = {
        ...
        results: [],
        searching: false,
        token: friendNameSearch.token + 1,
      }
...
function renderFriends(body) {
  body.innerHTML = `<div class="card" style="margin-bottom:18px"><h2>Invite study buddies</h2><p class="muted">Send this link — friends open it, sign up, and add you in seconds.</p><div class="input-row"><input class="input" id="invite-link" readonly value="${esc(inviteText())}"><button type="button" class="primary" id="copy-invite">Copy link</button><button type="button" class="ghost" id="share-invite">Share</button><button type="button" class="backend-pill probing" data-backend-status="friends" data-backend-retry title="Checking the backend…" aria-label="Backend status — click to retry"><i></i><span>Connecting…</span></button></div></div>${
    signedIn() ? cloudFriendsMarkup() : ""
  }${referralMarkup()}<div class="card"><h2>Friends &amp; gifting</h2><p class="muted">Add study partners here. They will also appear as gift recipients in the Rewards store.</p><div class="input-row" style="position:relative" id="friend-add-row"><input class="input" id="friend-name" placeholder="Username" aria-label="Friend username" autocomplete="off" role="combobox" aria-controls="friend-search-pop"><button type="button" class="primary" id="add-friend">Add friend</button><div class="friend-pick drop" id="friend-search-pop" role="listbox" hidden></div></div><div class="grid">${
    state.friends
      .map((f) => {
        const cf = cloudFriendReqsById(f.id)
        return `<div class="task" data-profile-user="${esc(f.id)}" data-profile-handle="${esc(f.username)}" data-profile-name="${esc(cf?.name || f.username)}" data-profile-photo="${esc(cf?.photo || "")}" data-profile-bio="${esc(cf?.bio || "")}" data-profile-state="friends"><div class="avatar">${friendAvatarMarkup(f.id, f.username, cf?.name)}</div><span class="task-text">@${esc(f.username)}</span><span class="friend-actions"><button type="button" class="ghost" data-chat-friend="${f.id}">Message</button><button type="button" class="ghost" data-block-friend="${f.id}">Block</button><button type="button" class="delete" data-remove-friend="${f.id}" title="Remove friend">×</button></span></div>`
      })
      .join("") ||
    '<p class="muted">Add a friend to send gifts and messages.</p>'
  }</div>${blockedSectionMarkup()}</div></div>`
  ...
  body
    .querySelector('[data-backend-retry][data-backend-status="friends"]')
    ?.addEventListener("click", async () => {
  ...
  $("#add-friend", body).onclick = async () => {
    ...
    if (
      (state.friends || []).some(
        (f) => (f.username || "").toLowerCase() === name.toLowerCase(),
      )
    )
    ...
    if (signedIn()) {
      try {
        ...
        const match =
          (data || []).find(
            (u) => String(u.handle || "").toLowerCase() === name.toLowerCase(),
          ) ||
          (data || []).find((u) =>
            String(u.handle || "")
              .toLowerCase()
        ...
        if (match) {
          ...
          if (reqErr) {
            notify(
              "Couldn't send request — " + String(reqErr.message).slice(0, 90),
            )
          } else {
            state.friends.push({
              id: match.id,
...
function openFriendsPicker({
  ...
  modal.innerHTML = `<div class="modal friends-picker"><div class="fp-head"><div><div class="eyebrow">${esc(eyebrow)}</div><h2>${esc(title)}</h2>${
    note ? `<p class="muted">${esc(note)}</p>` : ""
  }</div><span class="fp-count" data-fp-count></span></div><div class="fp-list" data-fp-list>${friends.map(fpRowHtml).join("")}</div><div class="modal-actions fp-actions"><button type="button" class="ghost" data-fp-cancel>Cancel</button><button type="button" class="primary" data-fp-done>${esc(cta)}</button></div></div>`
...
function fpRowHtml(f) {
  ...
  const dot =
    stories.length && !stories.every((s) => isSeen("cstory:" + s.id))
      ? '<span class="story-dot" title="New status"></span>'
      : ""
...
function fpToggle(id, row) {
  ...
  row.setAttribute(
    "aria-checked",
    friendPicker.selected.has(id) ? "true" : "false",
  )
...
document.addEventListener("click", (e) => {
  if (
    ...
    !e.target.closest?.("[data-profile-user]")
  )
    closeProfilePop()
})
...
function profileCardInner(d) {
  ...
  return `<div class="pp-head">${
    photo
      ...
      : `<span class="pp-ava">${esc(((d.handle || "?")[0] || "?").toUpperCase())}</span>`
  }<div class="pp-id"><strong>@${esc(d.handle)}</strong><small>${esc(d.name)}</small></div></div>${
    d.bio ? `<p class="pp-bio">${esc(d.bio)}</p>` : ""
  }${
    stateLabel ? `<span class="pp-state">${esc(stateLabel)}</span>` : ""
  }<button type="button" class="primary pp-view" data-pp-view>View profile</button>`
}
...
async function loadProfileFriends(modal, d) {
  ...
  try {
    ...
    rows.forEach((f) => {
      if (f.photo && !friendPhotoUrls.has(f.id))
        friendPhotoUrls.set(f.id, resolvePhoto(f.photo))
    })
    ...
    section.querySelector("[data-pv-friends-meta]").innerHTML = mutualN
      ? `<span class="pv-mutual-pill">${sicon("users")} ${mutualN} friend${
          mutualN === 1 ? "" : "s"
        } in common</span>`
    ...
    strip.innerHTML = rows
      .map((f) => {
        const state =
          f.mutual || cloudFriends.some((x) => x.id === f.id) ? "friends" : ""

        return (
          `<button type="button" class="pv-friend${
            f.mutual ? " mutual" : ""
          }" data-pv-friend data-profile-user="${esc(f.id)}" data-profile-handle="${esc(f.handle)}" data-profile-name="${esc(f.name)}" data-profile-photo="${esc(f.photo)}" data-profile-bio="${esc(f.bio)}" data-profile-state="${state}" title="View @${esc(f.handle)}">` +
          `<span class="pv-friend-ring${
            f.mutual ? " mutual" : ""
          }">${friendAvatarMarkup(f.id, f.handle, f.name)}</span>` +
...
function openProfileView(d) {
  ...
  modal.innerHTML = `<div class="modal profile-view"><span class="pv-cover"></span><div class="pv-head">${
    photo
      ? `<span class="pv-ava"><img src="${esc(photo)}" alt=""></span>`
      : `<span class="pv-ava">${esc(((d.handle || "?")[0] || "?").toUpperCase())}</span>`
  }<div class="pv-id"><h2>@${esc(d.handle)}</h2><p class="muted">${esc(d.name)}</p>${
    d.bio ? `<p class="pv-bio">${esc(d.bio)}</p>` : ""
  }</div></div><div class="pv-facts"><span class="tag">${
    isFriend
      ? sicon("check") + " Connected"
      : isPending
        ? "Request pending"
        : "Not connected"
  }</span>${
    stories.length
      ? `<span class="tag">${stories.length} status update${
          stories.length === 1 ? "" : "s"
        }</span>`
      : ""
  }</div>${
    canLoadFriends
      ? `<div class="pv-friends" data-pv-friends><div class="pv-friends-head"><span class="eyebrow">Friends</span><span class="pv-friends-meta" data-pv-friends-meta>Loading…</span></div><div class="pv-friends-strip pv-friends-loading" data-pv-friends-strip><span class="pv-friend-skel"></span><span class="pv-friend-skel"></span><span class="pv-friend-skel"></span></div></div>`
      : ""
  }${
    stories.length
      ? `<div class="pv-stories">${stories
          .slice(0, 6)
          .map(
            (s) =>
              `<button type="button" class="pv-story${
                isSeen("cstory:" + s.id) ? " seen" : ""
              }" data-pv-story="${esc(s.id)}" title="View status">${
                s.kind === "image"
                  ? `<img data-story-thumb="${esc(s.mediaPath)}" alt="">`
                  : `<span>${escSnippet(s.text || "", 60)}</span>`
              }<small>${relTime(s.createdAt)}</small></button>`,
          )
          .join("")}</div>`
      : ""
  }<div class="modal-actions pv-actions">${
    !isFriend && !isPending
      ? `<button type="button" class="primary" data-pv-add>Add friend</button>`
      : ""
  }${
    isFriend
      ? `<button type="button" class="ghost" data-pv-msg>Message</button><button type="button" class="ghost" data-pv-story-all>View status</button>`
      : ""
  }<button type="button" class="ghost" data-pv-close>Close</button>${
    isFriend
      ? `<button type="button" class="danger-button" data-pv-block>Block</button>`
      : ""
  }</div></div>`
...
function askBlockUser(ref) {
  ...
  if (
    (ref.username && ref.username === state.profile.handle) ||
    key === postOwnerId()
  )
  ...
  modal.innerHTML = `<div class="modal"><div class="eyebrow">Safety & privacy</div><h2>Block ${esc(name)}?</h2><p class="muted">Blocking will limit interactions between you and this account — no more direct messages, and their notes leave your feed.${
    existing
      ? " You have reported them before — continuing updates your report."
      : ""
  }</p><div class="modal-actions"><button type="button" class="ghost" data-block-cancel>Cancel</button><button type="button" class="danger-button" data-block-continue>Block User</button></div></div>`
...
function blockReasonDialog(ref) {
  ...
  modal.querySelectorAll('input[name="block-reason"]').forEach((radio) =>
    radio.addEventListener("change", () => {
      const show =
        modal.querySelector('input[name="block-reason"]:checked')?.value ===
        "Other"
  ...
  submitBtn.onclick = async () => {
    ...
    if (reason === "Other" && !complaint) {
      err.textContent =
        "Describe what happened, or pick a specific reason instead."
...
async function submitBlock(ref, reasons, complaint) {
  ...
  state.reports = [
    ...(state.reports || []).filter((r) => r.reportedKey !== key),
    report,
  ]

  if (ref.id)
    state.friends = (state.friends || []).filter((f) => f.id !== ref.id)
  ...
  notify(
    cloudOk
      ...
      : "User blocked — report saved on this device",
  )
...
function renderNotifications(body) {
  ...
  body.innerHTML = `<div class="card"><div class="section-row"><h2>Notifications</h2><span class="tag" data-notif-tag>${
    cloudNotifCount ? `${cloudNotifCount} unread` : "up to date"
  }</span></div><div data-notif-list><p class="muted">Loading…</p></div><div class="modal-actions"><button type="button" class="ghost" data-notif-read-all>Mark all read</button></div></div>`
  ...
  const paint = (list) => {
          ...
          .map(
            (n) =>
              `<div class="task"><div class="avatar">${sicon(n.type === "connection" ? "users" : n.type === "group_removed" ? "run" : "bell")}</div><span class="task-text"><strong>${esc(n.title)}</strong><br><small class="muted">${esc(n.text || "")} · ${notifTime(n.created_at)}</small></span><span class="friend-actions">${
                ...
                n.metadata?.group_id
                  ? `<button type="button" class="primary" data-notif-open="${n.id}">Open</button>`
                  : ""
              }</span></div>`,
          )
          .join("")
    ...
    box.querySelectorAll("[data-notif-open]").forEach(
      (b) =>
        (b.onclick = async () => {
  ...
  listNotifications(30)
    .then(({ data, error } = {}) => {
      if (error || !Array.isArray(data)) {
        ...
        if (box && !cloudNotifList.length)
          box.innerHTML =
            '<p class="muted">Couldn\'t load notifications — check your connection.</p>'
      ...
      if (state.tab === "community" && state.subtab === "notifications") {
        ...
        if (tag)
          tag.textContent = cloudNotifCount
            ? `${cloudNotifCount} unread`
            : "up to date"

        const tab = document.querySelector('[data-subtab="notifications"]')

        if (tab)
          tab.textContent = `Notifications${
            cloudNotifCount ? ` (${cloudNotifCount})` : ""
          }`
      }
    })
    .catch(() => {})
  ...
  body.querySelector("[data-notif-read-all]").onclick = async () => {
    ...
    cloudNotifList = cloudNotifList.map((n) => ({
      ...n,
...
function markChatRead(chatId) {
  ...
  const entry = {
    ts: Math.max(last?.ts || 0, Date.now()),
    id: last?.id || null,
  }
...
function chatDisplayInfo(id) {
  if (isSelfChat(id))
    return {
      ...
      target: {
        id: SELF_CHAT_ID,
        username: state.profile.handle || "me",
  ...
  const name = isGroup
    ? target?.name || "Group"
    : target?.name ||
      (target ? "@" + (target.username || target.handle) : "") ||
      "Chat"

  const photo = !isGroup
    ? resolvePhoto(
        target?.photo || (target?.cloud && cloudFriendPhoto?.(target.id)) || "",
      )
    : ""
...
function applyMsgSelectChrome(on) {
  ...
  if (!on) {
    chat
      .querySelectorAll(".bubble-row.select, .bubble-row.selected")
      .forEach((row) => {
...
function paintMsgSelectRow(mid) {
  ...
  const bubble = root?.querySelector(
    `[data-toggle-msg="${CSS.escape(String(mid))}"]`,
  )
...
function enterMsgSelectMode(chatId, preselectIds) {
  ...
  if (state.tab === "community" && state.subtab === "messages")
    renderCommunity()
...
function conversationRow(c) {
  ...
  const tick =
    last && last.me
      ? `<span class="conv-tick${
          state.messageStatus[last.id] === "read" ? " read" : ""
        }" aria-hidden="true">${sicon("checkDouble")}</span>`
      : ""

  const check = chatSelectMode
    ? `<span class="conv-check${
        selectedChats.has(c.id) ? " on" : ""
      }" aria-hidden="true">${
        selectedChats.has(c.id) ? sicon("check") : ""
      }</span>`
    : ""

  return (
    `<button type="button" class="conv-row ${active}${
      unread ? " has-unread" : ""
    }${chatSelectMode ? " select-mode" : ""}${
      chatSelectMode && selectedChats.has(c.id) ? " selected" : ""
    }" ${
      chatSelectMode
        ? `data-toggle-select="${c.id}"`
        : `data-select-chat="${c.id}"`
    }>` +
    check +
    convAvatarHtml(c, 49) +
    `<span class="conv-main">` +
    `<span class="conv-top"><span class="conv-name">${esc(c.name || "@" + c.username)}${
      muted
        ? ` <span class="mute-ico" title="Muted">${sicon("mute")}</span>`
        : ""
    }</span>` +
    `<span class="conv-time${unread ? " unread-time" : ""}">${
      last ? relTime(last.ts) : ""
    }</span></span>` +
    `<span class="conv-preview">${tick}${preview}</span></span>` +
    (unread && !chatSelectMode
      ? `<span class="conv-unread" title="${unread} unread message${
          unread === 1 ? "" : "s"
...
function renderMessages(body) {
  const cloudConns = cloudFriends
    ...
    .map((f) => ({
      ...
      photo: resolvePhoto(f.photo) || cloudFriendPhoto(f.id),
      cloud: true,
    }))
  ...
  const chats = [
    ...
    ...cloudConns.filter(
      (c) => !(state.friends || []).some((f) => f.id === c.id),
    ),
  ].filter(
    (c) =>
      !(state.deletedChats || {})[c.id] || (state.messages[c.id] || []).length,
  )

  if (
    state.activeChat &&
    (isBlockedKey(state.activeChat) || cloudBlocked.has(state.activeChat))
  )
    state.activeChat = null

  const totalUnread = chats.reduce(
    (sum, c) => sum + (state.activeChat === c.id ? 0 : unreadCount(c.id)),
    0,
  )
  ...
  body.innerHTML =
    `<div class="wa-root${state.activeChat ? " has-chat" : ""}${
      chatSelectMode ? " selecting" : ""
    }"><div class="wa-list">` +
    (chatSelectMode
      ? `<div class="wa-list-head wa-select-head"><h2>Select chats</h2><div class="wa-select-actions"><button type="button" class="ghost" data-select-cancel>Cancel</button><button type="button" class="delete" data-select-delete${
          selectedChats.size ? "" : " disabled"
        }>Delete${
          selectedChats.size ? ` (${selectedChats.size})` : ""
        }</button></div></div>`
      : `<div class="wa-list-head"><h2>Chats</h2>${
          signedIn() ? "" : `<span class="tag">local</span>`
        }</div>`) +
    messagesStoryStrip() +
    ...
    `<div class="chat wa-chat">${
      !chatSelectMode && state.activeChat
        ? groupSearch && groupSearch.id === state.activeChat
          ? groupSearchMarkup(state.activeChat)
          : chatMarkup(state.activeChat)
        : waPlaceholderMarkup()
    }</div></div>`
  ...
  $("[data-select-delete]", body)?.addEventListener(
    "click",
    deleteSelectedChats,
  )
  ...
  if (state.activeChat) {
    if (groupSearch && groupSearch.id === state.activeChat)
      bindGroupSearch(body, state.activeChat)
...
function messageAvatarHtml(id, m, isGroup) {
  ...
  if (m.me) {
    ...
    const letter = esc(
      (
        (state.profile.name || state.profile.handle || "Y")[0] || "Y"
      ).toUpperCase(),
    )
  ...
  const f =
    cloudFriends.find((x) => x.id === m.sender_id) ||
  ...
  return `<span class="bubble-ava">${
    src ? `<img src="${esc(src)}" alt="" loading="lazy">` : letter
  }</span>`
}
...
function bubbleRowHtml(chatId, m, i) {
  ...
  const check = `<span class="msg-check${
    selected ? " on" : ""
  }" aria-hidden="true">${selected ? sicon("check") : ""}</span>`

  const rowCls = `bubble-row${m.me ? " me" : ""}${selectOn ? " select" : ""}${
    selected ? " selected" : ""
  }`
  ...
  const inner = `<div class="bubble ${
    m.me ? "me" : ""
  }" data-toggle-msg="${esc(m.id)}" data-midx="${i}"${a11y}>${check}${messageHtml(m)}</div>`
...
function messageHtml(m) {
  ...
  const inviteMatch =
    !inviteMeta && typeof m.text === "string"
      ? m.text.match(
          /^(Sprint invite|Study session invite|Challenge invite|Sprint room open|New challenge|Scheduled:)/,
        )
      : null
  ...
  if (isInviteMsg && (m.kind == null || m.kind === "text")) {
    const t =
      inviteMeta?.t ||
      (m.text.startsWith("Challenge") || m.text.startsWith("New challenge")
        ? "challenge"
        : m.text.startsWith("Study session") || m.text.startsWith("Scheduled")
          ? "session"
          : "sprint")

    const icon =
      t === "challenge" ? "trophy" : t === "session" ? "calendar" : "bolt"
    ...
    body = `<div class="poll-q">${sicon("chart")} ${esc(m.question || "Poll")}</div>${m.options
      .map((opt, i) => {
        const pct = Math.round((counts[i] / (cast || 1)) * 100)

        return `<button type="button" class="poll-opt${
          mine === i ? " mine" : ""
        }" data-poll-vote="${m.id}:${i}"><span class="poll-bar" style="width:${pct}%"></span><span class="poll-label">${esc(opt.text)}${
          mine === i ? " " + sicon("check") : ""
        }</span><span class="poll-pct">${pct}%</span></button>`
      })
      .join("")}<div class="muted" style="font-size:11px">${cast} vote${
      cast === 1 ? "" : "s"
    } · tap to ${mine != null ? "change" : "cast"} your vote</div>`
  ...
  const chips = Object.entries(m.reactions || {})
    ...
    .map(
      ([emoji, users]) =>
        `<button type="button" class="react-chip${
          users.includes(key) ? " mine" : ""
        }" data-react="${m.id}:${esc(emoji)}" title="${esc(REACT_LABELS[emoji] || emoji)}">${REACT_LABELS[emoji] || sicon(emoji)} <span>${users.length}</span></button>`,
    )
  ...
  const canEdit =
    m.me && (m.kind == null || m.kind === "text") && typeof m.text === "string"

  return `${quote}${sysIcon}${body}<div class="bubble-tools"><small class="message-meta">${new Date(m.ts).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} ${status}${editedTag}</small><span class="bubble-actions"><button type="button" data-react-open="${m.id}" title="React">${sicon("smile")}</button><button type="button" data-reply-to="${m.id}" title="Reply">${sicon("reply")}</button><button type="button" data-pin="${m.id}" title="Pin message">${sicon("pin")}</button><button type="button" data-copy-msg="${m.id}" title="Copy message" aria-label="Copy message">${sicon("clip")}</button>${
    canEdit
      ? `<button type="button" data-edit-msg="${m.id}" title="Edit message" aria-label="Edit message">${sicon("memo")}</button>`
      : ""
  }${
    m.me
      ? `<button type="button" data-del-msg="${m.id}" title="Delete message">${sicon("trash")}</button>`
      : ""
  }</span></div>${
    chips ? `<div class="react-row">${chips}</div>` : ""
  }<div class="react-picker" data-picker="${m.id}" hidden>${REACT_EMOJI.map((e) => `<button type="button" data-react="${m.id}:${e}" title="${REACT_LABELS[e]}" aria-label="React ${REACT_LABELS[e]}">${REACT_LABELS[e]}</button>`).join("")}</div>`
}
...
function setChatMute(id, choiceId) {
  ...
  notify(
    c.id === "forever"
      ? `Muted ${g?.name || "group"} · always`
      : `Muted ${g?.name || "group"} · ${c.label}`,
  )
...
function groupRoster(id) {
  ...
  return [...roster.values()].sort(
    (a, b) => (b.you ? 1 : 0) - (a.you ? 1 : 0) || b.last - a.last,
  )
}
...
function groupMenuMarkup(id) {
  ...
  const item = (act, icon, label, sub) =>
    `<button type="button" data-gact="${act}" data-gid="${esc(id)}"><span class="gact-ico">${sicon(icon)}</span><span class="gact-txt"><strong>${label}</strong>${
      sub ? `<small>${sub}</small>` : ""
    }</span></button>`

  return (
    `${item("search", "search", "Search messages", "Find text, files and links")}` +
    ...
    `${
      owner
        ? item("disband", "trash", "Delete group", "Remove this group")
        : item("leave", "run", "Leave group", "Stop receiving messages")
    }`
  )
}
...
function dmMenuMarkup(id) {
  const isFriendChat =
    (state.friends || []).some((f) => f.id === id) ||
    cloudFriends.some((f) => f.id === id) ||
    isSelfChat(id)
  ...
  const social = self
    ...
    : item(
        ...
        "user",
        "View contact",
        "Profile, status and shared media",
      )
  ...
  return (
    ...
    `${
      muted
        ? item("unmute", "volume", "Unmute notifications", esc(muteLabel(id)))
        : item("mute", "mute", "Mute notifications", "Silence this chat")
    }` +
    `${social}` +
    `${
      self
    ...
    `${
      self || !isFriendChat
        ? ""
        : item("block", "lock", "Block", "Stop all contact")
    }`
  )
}
...
function chatMarkup(id) {
  ...
  const pinbar = pinned.length
    ? `<div class="pin-bar" role="list" aria-label="Pinned messages"><span class="pin-count">${sicon("pin")} ${pinned.length}</span><div class="pin-scroll" role="listbox">${pinned
        ...
        .join(
          "",
        )}</div><button type="button" class="icon-btn" data-pins-all title="All pinned messages" aria-label="Show all pinned messages">${sicon("list")}</button><button type="button" class="icon-btn" data-unpin-latest title="Unpin latest" aria-label="Unpin latest pinned message">×</button></div>`
    : ""
  ...
  const others = presenceInfo.chatId === id ? presenceInfo.users.length : 0

  const presence =
    backendConfigured && state.user
      ? `<span class="presence" title="People in this chat right now"><i class="${
          others ? "on" : ""
        }"></i>${others ? `${others} here now` : "only you here"}</span>`
      : ""
  ...
  const msgSelectBar = `<div class="msg-select-head" role="toolbar" aria-label="Select messages"${
    selecting ? "" : " hidden"
  }>
        <button type="button" class="ghost" data-msg-select-cancel>Cancel</button>
        <strong>${
          selCount ? `${selCount} selected` : "Select messages"
        }</strong>
        <span class="msg-select-actions">
          <button type="button" class="ghost" data-msg-select-all>Select all</button>
          <button type="button" class="delete" data-msg-select-delete${
            selCount ? "" : " disabled"
          }>Delete${selCount ? ` (${selCount})` : ""}</button>
        </span>
      </div>`
  const isFriendChat =
    (state.friends || []).some((f) => f.id === id) ||
  ...
  const mutedTag =
    (isGroup || isFriendChat) && isChatMuted(id)
      ? ' <span class="tag">muted</span>'
      : ""

  const nav =
    groupNav && groupNav.id === id && groupNav.matches.length
  ...
  return `${msgSelectBar}<div class="chat-head wa-head"><button type="button" class="wa-back" data-wa-back title="Back to chats" aria-label="Back to chats">${sicon("reply")}<span>Chats</span></button><button type="button" class="chat-head-who" data-chat-profile="${id}" title="Open profile"><span class="chat-ava">${
    isGroup
      ? groupAvatarMarkup(allGroups().find((g) => g.id === id))
      : headPhoto
...
function bindChat(root, id) {
  ...
  window.__sfChatVisRefresh = () => {
    if (state.activeChat === id)
      loadCloudHistory(id, chatRoot()).catch(() => {})
  }
  ...
  if (!window.__sfChatVisHook) {
    ...
    document.addEventListener("visibilitychange", () => {
      if (
        ...
        typeof window.__sfChatVisRefresh === "function"
      ) {
        window.__sfChatVisRefresh()
      }
    })
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        if (
          ...
          !e.target.closest?.("[data-chat-menu]")
        ) {
          open.hidden = true
        }
      },
  ...
  $$("[data-chat-block]", root).forEach(
    (b) =>
      (b.onclick = (e) => {
        ...
        const target =
          (state.friends || []).find((f) => f.id === b.dataset.chatBlock) ||
  ...
  $("[data-chat-profile]", root)?.addEventListener("click", (e) => {
    ...
    if (isSelfChat(id))
      return notify("This is your own space — notes to yourself live here.")
  ...
  $("#chat-file", root).onchange = (e) => {
    ...
    if (file.size > 3 * 1024 * 1024) return notify("Files must be under 3 MB")
  ...
  if (extrasToggle && extrasPop) {
    ...
    $("button[data-chat-extras-close]", root)?.addEventListener(
      "click",
      (e) => {
        e.stopPropagation()
    ...
    root.addEventListener("pointerdown", (e) => {
      ...
      const seek = (ev) => {
        ...
        if (activeVoice?.id === vid) {
          const dur =
            Number(widget.dataset.dur) || activeVoice.audio.duration || 1
  ...
  if (!root.dataset.bubbleDelegated) {
    ...
    root.addEventListener("click", (e) => {
      ...
      if (
        msgToggle &&
        root.contains(msgToggle) &&
        msgSelectMode &&
        msgSelectMode.chatId === cid
      ) {
        e.stopPropagation()
      ...
      if (pin && root.contains(pin)) {
        ...
        notify(
          wasPinned ? "Message unpinned" : sicon("pin") + " Message pinned",
        )
      ...
      if (reactOpen && root.contains(reactOpen)) {
        ...
        if (willOpen)
          $$("[data-picker]", root).forEach((p) => (p.hidden = true))
  ...
  $("[data-msg-select-cancel]", root)?.addEventListener(
    "click",
    exitMsgSelectMode,
  )

  $("[data-msg-select-all]", root)?.addEventListener("click", () =>
    selectAllMsgs(id),
  )

  $("[data-msg-select-delete]", root)?.addEventListener("click", () =>
    deleteSelectedMessages(id),
  )

  $$("[data-pin-jump]", root).forEach(
    (b) => (b.onclick = () => jumpToPinnedMessage(id, b.dataset.pinJump)),
  )
...
function fileIcon(mime, name) {
  ...
  if (
    /word|document|sheet|excel|csv|presentation|powerpoint|zip|officedocument/i.test(
      t,
    ) ||
    /\.(docx?|xlsx?|csv|pptx?|txt|md|zip|rar)$/i.test(n)
  )
    return "doc"
...
function groupMediaCounts(id) {
  ...
  return {
    media: s.images.length + s.videos.length,
...
function paintGroupMedia() {
  ...
  if (tab === "media")
    list = [
      ...s.images.map((x) => ({ ...x, kind: "image" })),
      ...s.videos.map((x) => ({ ...x, kind: "video" })),
    ].sort((a, b) => b.ts - a.ts)
  ...
  const head = (t, label, n) =>
    `<button type="button" class="filter ${
      tab === t ? "active" : ""
    }" data-gm-tab="${t}">${label} (${n})</button>`
  ...
  if (!list.length) {
    ...
    body = `<div class="media-grid">${shown
      .map((it, i) =>
        it.kind === "image"
          ? `<button type="button" class="media-thumb" data-gm-view="${i}" title="${esc(it.name)}"><img src="${it.url}" alt="${esc(it.name)}" loading="lazy"></button>`
          : `<button type="button" class="media-thumb" data-gm-view="${i}" title="${esc(it.name)}"><video src="${it.url}" preload="metadata" muted playsinline></video><span class="media-play">${sicon("play")}</span></button>`,
      )
      .join("")}</div>`
  } else if (tab === "files") {
  ...
  if (rest > 0)
    body += `<div style="text-align:center;margin-top:12px"><button type="button" class="ghost" data-gm-more>Show more (${rest} remaining)</button></div>`
  ...
  $$("[data-gm-view]", modal).forEach(
    (b) => (b.onclick = () => openMediaViewer(id, Number(b.dataset.gmView))),
  )
...
function openMediaViewer(id, pos) {
  ...
  mediaViewer = {
    items,
    pos: Math.min(Math.max(0, pos), items.length - 1),
    title: groupById(id)?.name || "Group",
  }
...
function paintMediaViewer() {
  ...
  modal.innerHTML = `<div class="viewer"><div class="viewer-head"><span class="muted">${esc(title)} · ${pos + 1} / ${items.length}</span><button type="button" class="icon-btn" data-mv-close aria-label="Close viewer">×</button></div><div class="viewer-body">${
    it.kind === "image"
      ? `<img src="${it.url}" alt="${esc(it.name)}">`
      : `<video src="${it.url}" controls autoplay playsinline></video>`
  }<button type="button" class="viewer-arrow left" data-mv-prev aria-label="Previous">‹</button><button type="button" class="viewer-arrow right" data-mv-next aria-label="Next">›</button></div><div class="viewer-foot muted">${esc(it.name)} · ${esc(it.sender)} · ${fmtSharedDate(it.ts)}</div></div>`
...
function dmAction(act, id, root) {
  ...
  if (act === "contact")
    return isSelfChat(id)
      ? notify("This is your own space — notes to yourself live here.")
      : openContactCard(id, info)

  if (act === "clear") {
    return confirmBox(
      `Clear ${isSelfChat(id) ? "your notes" : name} history?`,
      "Messages on this device will be removed. New messages still arrive.",
      () => {
        const msgs = { ...(state.messages || {}) }
  ...
  if (act === "block") {
    ...
    return askBlockUser({
      ...
      name: info.name.startsWith("@")
        ? info.name
        : "@" + (target.username || target.handle || info.name),
...
function openDmMuteModal(id, name) {
  ...
  modal.innerHTML = `<div class="modal"><div class="eyebrow">Notifications</div><h2>Mute ${esc(name)}?</h2><p class="muted">You can still open the chat and read everything. Only notifications pause.${
    isChatMuted(id) ? ` Currently: <strong>${esc(muteLabel(id))}</strong>.` : ""
  }</p><div class="reason-list">${MUTE_CHOICES.map((c) => `<label><input type="radio" name="mute-choice" value="${c.id}"${cur === "forever" && c.id === "forever" ? " checked" : ""}> ${c.id === "forever" ? "Always (until turned back on)" : c.label}</label>`).join("")}</div><p class="st-confirm-err" data-mute-err hidden></p><div class="modal-actions"><button type="button" class="ghost" data-mute-cancel>Cancel</button>${
    isChatMuted(id)
      ? `<button type="button" class="ghost" data-mute-off>Unmute</button>`
      : ""
  }<button type="button" class="primary" data-mute-save>Save</button></div></div>`
  ...
  modal.querySelector("[data-mute-save]").onclick = () => {
    const choice =
      modal.querySelector('input[name="mute-choice"]:checked')?.value || ""
...
function openDmReport(id, name) {
  ...
  modal.querySelector("[data-dm-report-send]").onclick = async () => {
    const reason =
      modal.querySelector('input[name="dm-report-reason"]:checked')?.value || ""
    ...
    const details = modal
      .querySelector("[data-dm-report-details]")
      .value.trim()
      .slice(0, 2000)
    ...
    state.reports = [
      ...(state.reports || []).filter(
        (r) => r.reportedKey !== report.reportedKey,
      ),
      report,
    ]
    ...
    notify(
      cloudOk
        ? "Report sent — moderation will review"
...
function openContactCard(id, info) {
  ...
  const sharedDays = msgs.length
    ? Math.max(
        1,
        Math.ceil(
          (Date.now() - Math.min(...msgs.map((m) => m.ts || Date.now()))) /
            86400000,
        ),
      )
    : 0
  ...
  modal.innerHTML = `<div class="modal contact-card"><div class="contact-hero"><span class="contact-avatar">${avatarMarkup(photo, (info.name.replace(/^@/, "")[0] || "?").toUpperCase())}</span><h2>${esc(info.name)}</h2>${
    target.username && !info.name.startsWith("@")
      ? `<small class="muted">@${esc(target.username)}</small>`
      : ""
  }${
    msgs.length
      ? `<p class="muted contact-sub">${msgs.length} message${
          msgs.length === 1 ? "" : "s"
        } shared${
          sharedDays
            ? ` over ~${sharedDays} day${sharedDays === 1 ? "" : "s"}`
            : ""
        }</p>`
      : '<p class="muted contact-sub">No messages yet</p>'
  }</div><div class="book-facts"><span class="tag">${counts.media} media</span><span class="tag">${counts.files} files</span><span class="tag">${counts.links} links</span>${
    isChatMuted(id) ? `<span class="tag">${esc(muteLabel(id))}</span>` : ""
  }</div><div class="modal-actions contact-actions"><button type="button" class="ghost" data-cc-mute>${
    isChatMuted(id) ? "Unmute" : "Mute"
...
function openMuteModal(id) {
  ...
  modal.innerHTML = `<div class="modal"><div class="eyebrow">Notifications</div><h2>Mute ${esc(g.name)}?</h2><p class="muted">You can still open the group and read everything. Only notifications pause.${
    isChatMuted(id) ? ` Currently: <strong>${esc(muteLabel(id))}</strong>.` : ""
  }</p><div class="reason-list">${MUTE_CHOICES.map((c) => `<label><input type="radio" name="mute-choice" value="${c.id}"${cur === "forever" && c.id === "forever" ? " checked" : ""}> ${c.id === "forever" ? "Always (until turned back on)" : c.label}</label>`).join("")}</div><p class="st-confirm-err" data-mute-err hidden></p><div class="modal-actions"><button type="button" class="ghost" data-mute-cancel>Cancel</button>${
    isChatMuted(id)
      ? `<button type="button" class="ghost" data-mute-off>Unmute</button>`
...
function openGroupMembers(id) {
  ...
  modal.innerHTML = `<div class="modal"><div class="eyebrow">Members</div><h2>${esc(g.name)}</h2><div data-cloud-roster><p class="muted">Loading members…</p></div>${
    typeof g.members === "number" && !g.ownerId
      ? `<p class="muted">About ${g.members} learners follow this group. Recently active here:</p>`
      : `<p class="muted">People who have sent messages here:</p>`
  }${
    roster.length
      ? `<div class="grid">${roster.map((r) => `<div class="task"><div class="avatar">${r.you ? esc((state.profile.handle || "Y")[0].toUpperCase()) : "M"}</div><span class="task-text"><strong>${r.you ? "You" : "Group member"}</strong><br><small class="muted">${r.count} message${r.count === 1 ? "" : "s"}</small></span><span>${r.you ? `<span class="tag">you</span>` : ""}${r.you && g.ownerId === chatKey() ? `<span class="tag">owner</span>` : ""}${!r.you && g.ownerId && r.key === `other:${g.ownerId}` ? `<span class="tag">owner</span>` : ""}</span></div>`).join("")}</div>`
      : `<p class="muted">No messages here yet — members appear once they write.</p>`
  }<div class="modal-actions"><button type="button" class="primary" data-members-close>Done</button></div></div>`
...
async function paintCloudRoster(modal, id) {
  ...
  try {
    ...
    const myRole =
      (members.find((m) => m.user_id === me) || {}).role || myGroupRole(id)
    ...
    box.innerHTML = `<div class="section-row" style="margin-top:4px"><h3>Members (${members.length})</h3>${
      myRole ? `<span class="tag">you: ${esc(myRole)}</span>` : ""
    }</div><div class="grid">${members
      .map((m) => {
        const you = m.user_id === me

        const actions =
          !you && canAdmin && m.role !== "owner"
            ? `<span class="friend-actions">${
                myRole === "owner" && m.role === "member"
                  ? `<button type="button" class="ghost" data-m-promote="${m.user_id}">Make admin</button>`
                  : ""
              }${
                myRole === "owner" && m.role === "admin"
                  ? `<button type="button" class="ghost" data-m-demote="${m.user_id}">Demote</button>`
                  : ""
              }<button type="button" class="ghost" data-m-remove="${m.user_id}">Remove</button>${
                myRole === "owner"
                  ? `<button type="button" class="ghost" data-m-transfer="${m.user_id}" title="Give them ownership">Transfer</button>`
                  : ""
              }</span>`
            : ""

        return `<div class="task"><div class="avatar">${esc(((m.handle || m.name || "?")[0] || "?").toUpperCase())}</div><span class="task-text"><strong>${
          you ? "You" : "@" + esc(m.handle || "member")
        }</strong><br><small class="muted">${esc(m.name || "")}</small></span><span class="tag">${esc(m.role)}</span>${actions}</div>`
      })
      .join("")}</div>${
      canAdmin
        ? `<div class="input-row" style="margin-top:10px"><input class="input" data-m-invite placeholder="Username to invite" aria-label="Username to invite"><button type="button" class="primary" data-m-invite-btn>Invite</button></div><p class="muted" style="margin:6px 0 0">Invites work for private groups too — only owners and admins can add people.</p>`
        : ""
    }`
    ...
    box.querySelectorAll("[data-m-promote]").forEach(
      (b) =>
        (b.onclick = async () => {
          const { error } = await groupSetMember(
            id,
            b.dataset.mPromote,
            "promote",
          )
          ...
          notify(
            error
              ? "Couldn't promote — " + String(error.message).slice(0, 100)
              : "Promoted to admin",
          )
    ...
    box.querySelectorAll("[data-m-demote]").forEach(
      (b) =>
        (b.onclick = async () => {
          const { error } = await groupSetMember(
            id,
          ...
          notify(
            error
              ? "Couldn't demote — " + String(error.message).slice(0, 100)
              : "Demoted to member",
          )
    ...
    box.querySelectorAll("[data-m-remove]").forEach(
      (b) =>
        (b.onclick = () => {
          confirmBox(
            "Remove this member?",
            "They lose access to the group and its chat.",
            async () => {
              const { error } = await groupSetMember(
                id,
                b.dataset.mRemove,
              ...
              notify(
                error
                  ? "Couldn't remove — " + String(error.message).slice(0, 100)
                  : "Member removed",
              )
    ...
    box.querySelectorAll("[data-m-transfer]").forEach(
      (b) =>
        (b.onclick = () => {
          confirmBox(
            "Transfer ownership?",
            "They become the group owner and you become an admin. This can't be undone by you.",
            async () => {
              const { error } = await groupTransfer(id, b.dataset.mTransfer)

              notify(
                error
                  ? "Couldn't transfer — " + String(error.message).slice(0, 100)
    ...
    if (inviteBtn)
      inviteBtn.onclick = async () => {
        ...
        try {
          ...
          if (!found.startsWith("Couldn't") && !found.startsWith("No user"))
            refresh()
        } finally {
...
async function inviteMemberByHandle(groupId, handle) {
  ...
  if (sErr)
    return "Couldn't search right now — " + String(sErr.message).slice(0, 80)

  const match =
    (results || []).find(
      (u) => String(u.handle || "").toLowerCase() === handle.toLowerCase(),
    ) || (results || [])[0]
...
function openGroupSettings(id) {
  ...
  modal.querySelector("[data-gs-save]").onclick = () => {
    ...
    g.description = modal
      .querySelector("[data-gs-desc]")
      .value.trim()
      .slice(0, 500)

    g.tags = modal
      .querySelector("[data-gs-tags]")
      .value.split(",")
      .map((x) => x.trim())
      .filter(Boolean)
      .slice(0, 8)
...
function openCloudGroupSettings(id, g, role) {
  ...
  modal.innerHTML = `<div class="modal"><div class="eyebrow">Group settings · ${esc(role)}</div><h2>${esc(g.name)}</h2><div class="group-avatar-row"><span data-av-settings>${groupAvatarMarkup(g)}</span><div><div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" class="ghost" data-av-change>Change photo</button>${
    g.avatarPath
      ? `<button type="button" class="ghost" data-av-remove>Remove</button>`
      : ""
  }</div><p class="muted" style="margin:6px 0 0">JPEG, PNG, WebP or GIF · under 2 MB · visible to members.</p><input type="file" data-av-file accept="image/jpeg,image/png,image/webp,image/gif" hidden aria-label="Choose a group photo"></div></div><label class="field-label">Name<input class="input" data-gs-name value="${esc(g.name)}" maxlength="120"></label><label class="field-label">Description<textarea class="textarea autogrow" data-gs-desc rows="2" maxlength="500">${esc(g.description || "")}</textarea></label><label class="field-label">Topics (comma separated)<input class="input" data-gs-tags value="${esc((g.tags || []).join(", "))}" maxlength="200"></label><label class="field-label">Icon<select class="select" data-gs-logo>${GROUP_LOGOS.map((n) => `<option value="${n}"${g.logoName === n ? " selected" : ""}>${n[0].toUpperCase() + n.slice(1)}</option>`).join("")}</select></label>${
    role === "owner"
      ? `<label class="field-label">Visibility<select class="select" data-gs-vis><option value="private"${
          g.visibility !== "public" ? " selected" : ""
        }>Private — invite only</option><option value="public"${
          g.visibility === "public" ? " selected" : ""
        }>Public — listed in Discover</option></select></label>`
      : ""
  }<p class="st-confirm-err" data-gs-err hidden></p><div class="modal-actions"><button type="button" class="ghost" data-gs-cancel>Cancel</button><button type="button" class="primary" data-gs-save>Save</button></div></div>`
  ...
  avFile?.addEventListener("change", async () => {
    ...
    try {
      ...
      if (prev && prev !== data.path)
        await removeGroupAvatar(prev).catch(() => {})
  ...
  modal.querySelector("[data-gs-save]").onclick = async () => {
    ...
    if (visSel && role === "owner")
      patch.visibility = visSel.value === "public" ? "public" : "private"
...
function openGroupReport(id) {
  ...
  modal.querySelector("[data-group-report-cancel]").onclick = () =>
    modal.remove()
  ...
  modal.querySelector("[data-group-report-send]").onclick = async () => {
    const reason =
      modal.querySelector('input[name="group-report-reason"]:checked')?.value ||
      ""
    ...
    const details = modal
      .querySelector("[data-group-report-details]")
      .value.trim()
      .slice(0, 2000)
    ...
    notify(
      cloudOk
        ? "Group reported — moderation will review"
        : "Report saved on this device",
    )
  }
}
...
function askLeaveGroup(id) {
  ...
  confirmBox(
    `Leave ${g.name}?`,
    `You stop receiving its messages. Your sent messages stay visible to others.${ownerNote}`,
    async () => {
      if (signedIn() && (isCloudGroup(id) || role)) {
        try {
          ...
          if (data?.deleted_group)
            notify(
              `Left ${g.name} — the group was removed (you were the only member)`,
            )
          else if (data?.transferred_to)
            notify(`Left ${g.name} — ownership transferred`)
...
function askDisbandGroup(id) {
  ...
  const cloudOwned =
    isCloudGroup(id) &&
    (role === "owner" ||
      (g?.ownerId && state.user && g.ownerId === state.user.id))

  if (!g || (!isGroupOwner(id) && !cloudOwned))
    return notify("Only the group owner can delete it")

  confirmBox(
    `Delete ${g.name}?`,
    "The group disappears for everyone, along with its messages.",
    async () => {
...
function groupAction(act, id, root) {
  ...
  if (act === "search") openGroupSearch(id)
    ...
    if (isGroupOwner(id))
      return notify("You own this group — use settings instead")
...
if (!window.__sfGroupKeysBound) {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    const extras = document.querySelector(
      "[data-chat-extras-pop]:not([hidden])",
    )
...
function openPinnedList(id) {
  ...
  modal.innerHTML = `<div class="modal pins-modal"><div class="eyebrow">${esc(target?.name || "Chat")}</div><h2>${sicon("pin")} Pinned messages</h2>${
    pinned.length
      ? `<div class="pins-list">${pinned
          ...
          .join("")}</div>`
      : '<p class="muted">Nothing pinned yet. Hover a message and press the pin icon.</p>'
  }<div class="modal-actions"><button type="button" class="primary" data-pins-close>Done</button></div></div>`
...
function snippetHtml(text, needle) {
  ...
  return `${
    s > 0 ? "…" : ""
  }${esc(t.slice(s, i))}<mark>${esc(t.slice(i, i + n.length))}</mark>${esc(t.slice(i + n.length, e))}${
    e < t.length ? "…" : ""
  }`
}
...
function searchGroupMessages(id, q) {
  ...
  for (const [idx, m] of visibleMsgs(id).entries()) {
    ...
    const hay =
      `${text} ${m.file || ""} ${senderLabel(m)} ${extractLinks(text).join(" ")}`.toLowerCase()
...
function groupSearchMarkup(id) {
  ...
  return `<div class="chat-head wa-head"><button type="button" class="icon-btn" data-gs-back aria-label="Back to chat">‹</button><div style="flex:1;min-width:0"><strong>${esc(info.name || "Chat")}</strong><div class="muted" style="font-size:11px">Search messages</div></div><span class="tag">${results.length}</span></div><div class="input-row" style="margin:12px 14px 0"><span class="song-search-ico" style="position:static;transform:none" aria-hidden="true">${sicon("search")}</span><input class="input" id="gs-input" style="flex:1" placeholder="Search messages, files, links…" value="${esc(q)}" aria-label="Search messages" autocomplete="off"></div>${
    signedIn() &&
    !isSelfChat(id) &&
    q.trim().length >= 2 &&
    (groupSearch?.depth || 0) < 4 &&
    targetKind(id) !== "friend"
      ? `<div style="margin:8px 14px 0" data-gs-more-wrap><button type="button" class="ghost" data-gs-more ${
          groupSearch?.deepening ? "disabled" : ""
        }>${
          groupSearch?.deepening
            ? "Searching older messages…"
            : "Search older messages"
        }</button></div>`
      : `<div data-gs-more-wrap></div>`
  }<div class="chat-body" id="gs-results">${groupSearchResultsHtml(id, results)}</div>`
}
...
function groupSearchResultsHtml(id, results) {
  if (!results.length) {
    ...
    return `<p class="muted" style="padding:16px">${
      q
        ? "No messages match that search."
        : "Type above to search this group's stored messages."
    }</p>`
  }
  ...
  return results
    .map(
      (r) =>
        `<button type="button" class="gs-row" data-gs-jump="${r.idx}"><span class="gs-who">${esc(r.sender)} · ${new Date(r.ts).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}${
          r.tags.length ? ` · ${r.tags.join(" · ")}` : ""
        }</span><span class="gs-snippet">${r.snippet}</span></button>`,
    )
    .join("")
}
...
function paintGroupSearchResults(id) {
  ...
  if (wrap) {
    const show =
      signedIn() && q.trim().length >= 2 && (groupSearch?.depth || 0) < 4

    wrap.innerHTML = show
      ? `<button type="button" class="ghost" data-gs-more ${
          groupSearch?.deepening ? "disabled" : ""
        }>${
          groupSearch?.deepening
            ? "Searching older messages…"
...
function openPollBuilder(id, root) {
  ...
  const optionRow = (n) =>
    `<div class="poll-opt-row" data-poll-row><span class="poll-opt-num">${n}</span><input class="input" data-poll-opt placeholder="Option ${n}" maxlength="80"><button type="button" class="icon-btn poll-opt-del" data-poll-row-del title="Remove option" aria-label="Remove option ${n}">${sicon("x")}</button></div>`
  ...
  const sync = () => {
    ...
    rows().forEach((row, i) => {
      ...
      if (del) {
        ...
        del.title =
          n <= 2 ? "A poll needs at least two options" : "Remove option"
      }
    })
  ...
  const addRow = (focus = true) => {
    ...
    if (focus)
      list.querySelector("[data-poll-row]:last-child [data-poll-opt]")?.focus()
  }
    ...
    .then((stream) => {
      ...
      rec.onstop = () => {
        ...
        const speechMs = Math.max(
          1000,
          (voiceRec?.tickerAt || Date.now()) - startedAt,
        )
      ...
      voiceRec = {
        ...
        timer: setInterval(
          () => {
            ...
            const s = Math.max(
              0,
              Math.floor((voiceRec.tickerAt - voiceRec.startedAt) / 1000),
            )
            ...
            if (hint) {
              if (voiceRec.paused)
                hint.textContent = "Paused — tap play to keep recording"
              else {
                const size =
                  chunks.reduce((a, c) => a + (c.size || 0), 0) / (1024 * 1024)
...
function subscribePresenceFor(id) {
  ...
  presenceSub = subscribeToPresence(
    ...
    (users) => {
      ...
      const prev = ((id === presenceInfo.chatId && presenceInfo.users) || [])
        .map((u) => u.user_id)
        .sort()
        .join(",")
      ...
      if (sig !== prev && state.tab === "community" && state.activeChat === id)
        renderCommunity()
    },
  )
}
...
function paintChatBody(id, root, scrollBottom) {
  ...
  body.innerHTML =
    msgs.map((m, i) => bubbleRowHtml(id, m, i)).join("") ||
    '<span class="muted">No messages yet. Start the conversation.</span>'
...
async function resolveSenderName(userId) {
  ...
  const known =
    cloudFriends.find((f) => f.id === userId) ||
    cloudFriendReqs.find((r) => r.otherId === userId)
...
async function resolveSenderNames(rows, me) {
  ...
  const { data: profiles } = await getPublicProfiles(unknown).catch(() => ({
    data: [],
  }))

  return new Map(
    (profiles || []).map((p) => [p.id, "@" + (p.handle || "member")]),
  )
}
  ...
  if (added) {
    ...
    harvestInvitesFromRows(rows, (r) => {
      ...
      return {
        id: r.sender_id,
        handle:
          names.get(r.sender_id)?.replace(/^@/, "") || p.handle || "a friend",
      }
    })
  }
...
async function attachCloudReactions(id, me) {
  const cloudIds = (state.messages[id] || [])
    .map((m) => m.cloudId)
    .filter(Boolean)

  if (!cloudIds.length) return false

  const { data, error } = await loadMessageReactions(cloudIds).catch(() => ({
    data: [],
  }))
  ...
  for (const m of state.messages[id] || []) {
    ...
    if (
      JSON.stringify(m.reactions || {}) !== JSON.stringify(byMsg.get(m.cloudId))
    ) {
...
async function deepenGroupSearch(id) {
  ...
  const oldest = local.reduce(
    (n, m) => Math.min(n, m.ts || Date.now()),
    Date.now(),
  )
      ...
      onMessage: (message) => {
        ...
        if (message.sender_id === state.user?.id) {
          ...
          if (match) {
            ...
            if (match.ts && message.created_at)
              match.ts = new Date(message.created_at).getTime()
        ...
        const dupe = (state.messages[id] || []).some(
          (m) => m.id === message.id || m.cloudId === message.id,
        )
        ...
        harvestInvitesFromRows([message], (r) => ({
          id: r.sender_id,
          handle: cloudFriendReqsById(r.sender_id)?.handle || "a friend",
        }))
        ...
        const attach = async () => {
          ...
          const twin = (state.messages[id] || []).some(
            (m) => m.id === message.id || m.cloudId === message.id,
          )
          ...
          if (state.activeChat !== id) {
            ...
            if (!twin) {
              state.messages[id] = [...(state.messages[id] || []), row].slice(
                -300,
              )
            ...
            if (state.tab === "community" && !isChatMuted(id))
              paintMessagesBadge(totalUnreadCount())
          } else {
            if (!twin) {
              state.messages[id] = [...(state.messages[id] || []), row].slice(
            ...
            if (
              !twin &&
              !appendChatBubble(id, row, chatRoot()) &&
              state.tab === "community"
            )
              renderCommunity()
          ...
          if (state.user)
            markMessageRead(message.id, state.user.id).catch(() => {})
          ...
          if (notifOn("community") && !isChatMuted(id)) {
            ...
            const who =
              chat?.name ||
              (chat ? "@" + (chat.username || chat.handle) : "Community")
...
function askDeleteMessage(id, mid, root) {
  ...
  if (!msg.me && !canModerate)
    return notify("You can only delete your own messages")

  confirmBox(
    canModerate && !msg.me ? "Delete this message?" : "Delete this message?",
    canModerate && !msg.me
      ? "Removed for everyone in the group."
      : "Removed from this conversation.",
    async () => {
      state.messages[id] = (state.messages[id] || []).filter(
        (m) => m.id !== mid,
      )
      ...
      const bubble = document
        .querySelector(`[data-react-open="${mid}"]`)
        ?.closest(".bubble")
...
function startMessageEdit(cid, mid, root) {
  ...
  if (msg.kind != null && msg.kind !== "text")
    return notify("Only text messages can be edited")
  ...
  bubble.querySelector("[data-msg-edit-cancel]").onclick = () =>
    cancelMessageEdit()
...
async function saveMessageEdit() {
  ...
  const bubble =
    root.querySelector(`[data-react-open="${mid}"]`)?.closest(".bubble") ||
  ...
  if (msg.cloudId && signedIn()) {
    ...
    try {
      ...
      notify(
        /permission|forbidden|GROUP_FORBIDDEN/i.test(raw)
...
function renderCall() {
  ...
  const peerPhoto =
    state.call.photo ||
    (state.call.peerId ? friendPhotoUrl(state.call.peerId) : "") ||
    ""

  const peerInitial = esc(
    String(state.call.name || "P")
      .replace(/^@/, "")
      .charAt(0)
      .toUpperCase() || "P",
  )
  ...
  const peerAvatarHtml = (cls = "") =>
    `<div class="tile-avatar${cls ? " " + cls : ""}">${
      peerPhoto
        ? `<img src="${esc(peerPhoto)}" alt="" loading="lazy">`
        : esc(state.call.emoji || peerInitial)
    }</div>`
  ...
  const remoteTiles = [...callRemotes.entries()]
    .map(([pid, stream], i) => {
      ...
      const statusBit =
        pStatus && pStatus !== "connected"
          ? ` <em class="part-st st-${esc(pStatus)}">${esc(pStatus)}</em>`
          : ""
      ...
      return `<div class="call-tile remote" data-remote-tile="${esc(pid)}">${
        stream && !isVoice
          ...
          : `${peerAvatarHtml()}${
              stream
                ? ""
                : `<div class="tile-hint"><span class="call-dots"><i></i><i></i><i></i></span> Connecting…</div>`
            }`
  ...
  const partnerTile = callRemotes.size
    ...
    : `<div class="call-tile main">${peerAvatarHtml()}<span class="tile-tag">${sicon(isVoice ? "mic" : "user")} ${esc(state.call.name || (isGroup ? "Room" : "Partner"))}</span><div class="tile-hint">${
        dialing
          ? `<span class="call-dots"><i></i><i></i><i></i></span> ${
              state.call.kind === "dm" && callIsInitiator
                ? `Calling ${esc(state.call.name || "partner")}…`
  ...
  const selfTile =
    activeCallStream && !isVoice
      ? `<video class="call-video mirrored" data-call-self autoplay playsinline muted></video>`
      : `<div class="tile-avatar small">${
          selfPhoto
            ? `<img src="${esc(selfPhoto)}" alt="">`
            : esc(state.profile.avatar || state.profile.name?.[0] || "SL")
        }</div>`

  const stageInner =
    isGroup && callRemotes.size
      ? `${
          selfTile
            ? `<div class="call-tile self-inline">${selfTile}<span class="tile-tag">${
                state.callMuted ? sicon("mute") + " Muted" : "You"
              }</span></div>`
            : ""
        }${remoteTiles}`
      : `<div class="call-tile main">${partnerTile.replace(/^<div class="call-tile main">|<\/div>$/g, "")}</div>
       <div class="call-tile self">${selfTile}<span class="tile-tag">${
         state.callMuted ? sicon("mute") + " Muted" : "You"
       }</span></div>`
  ...
  if (isGroup || callParticipantStates.size > 2) {
    ...
    if (chips)
      rosterStrip = `<div class="call-roster" aria-label="Participants">${chips}</div>`
  }

  call.innerHTML = `
    <div class="call-head">
      <span class="call-live"><i class="call-dot"></i> ${esc(state.call.name)}${
        isVoice ? " · voice" : ""
      }${isGroup ? ` · ${callRemotes.size + 1}` : ""}</span>
      ...
      <span class="call-head-actions">
        ${
          state.callStatus === "connected"
            ? `<button type="button" class="call-icon" data-call-add title="Add people" aria-label="Add people to call">${sicon("users")}</button>`
            : ""
        }
        <button type="button" class="call-icon" data-call-pips title="${
          minimized ? "Expand call" : "Minimize"
        }" aria-label="${minimized ? "Expand call" : "Minimize call"}">${
          minimized ? sicon("expand") : sicon("tab")
        }</button>
        <button type="button" class="call-icon danger" data-end title="End call" aria-label="End call">${sicon("x")}</button>
      </span>
    </div>
    ${rosterStrip}
    <div class="call-stage ${isGroup ? "group" : ""}${
      isVoice ? " voice" : ""
    }" data-call-stage>
      ${stageInner}
      ${
        state.callCameraOff && activeCallStream && !isVoice
          ? '<div class="cam-off-note">Camera is off</div>'
          : ""
      }
      ${
        connecting
          ? `<div class="call-connecting"><span class="call-dots"><i></i><i></i><i></i></span><p>${
              isVoice ? "Connecting audio…" : "Connecting…"
            }</p><p class="sub">${
              isVoice ? "audio only" : "audio + video"
            }</p></div>`
          : ""
      }
      ${
        dialing
          ? `<div class="call-connecting"><span class="call-dots"><i></i><i></i><i></i></span><p>${
              state.call.kind === "dm" && callIsInitiator
                ? `Calling ${esc(state.call.name || "partner")}…`
                : "Ringing…"
            }</p><p class="sub">Waiting for an answer</p></div>`
          : ""
      }
    </div>
      ...
      <div class="call-ctrl-group">
        ${
          state.callStatus === "connected"
            ...
            : dialing || connecting
              ? ""
              : `<button type="button" class="call-btn primary tall" data-connect>${sicon(isVoice ? "mic" : "phone")}${
                  isVoice ? " Start voice call" : " Start call"
                }</button>`
        }
      </div>
      <div class="call-ctrl-group">
        <button type="button" class="call-btn round ${
          state.callMuted ? "off" : ""
        }" data-mute title="${
          state.callMuted ? "Unmute microphone" : "Mute microphone"
        }" aria-pressed="${Boolean(state.callMuted)}" aria-label="Microphone">${
          state.callMuted ? sicon("mute") : sicon("mic")
        }</button>
        ${
          isVoice
            ? ""
            : `<button type="button" class="call-btn round ${
                state.callCameraOff ? "off" : ""
              }" data-camera title="${
                state.callCameraOff ? "Turn camera on" : "Turn camera off"
              }" aria-pressed="${Boolean(state.callCameraOff)}" aria-label="Camera">${
                state.callCameraOff ? sicon("mute") : sicon("camera")
              }</button>
  ...
  $(`[data-call-add]`, call)?.addEventListener("click", () =>
    inviteToActiveCall(),
  )
  ...
  $$("[data-call-reaction]", call).forEach(
    (b) =>
      (b.onclick = () =>
        pushCallReaction(
          b.dataset.callReaction === "fire"
            ? "🔥"
            : b.dataset.callReaction === "party"
              ? "🎉"
              : "💪",
        )),
  )
  ...
  $(`[data-mute]`, call).onclick = () => {
    ...
    activeCallStream
      ?.getAudioTracks()
      .forEach((t) => (t.enabled = !state.callMuted))
  ...
  $(`[data-camera]`, call)?.addEventListener("click", () => {
    ...
    activeCallStream
      ?.getVideoTracks()
      .forEach((t) => (t.enabled = !state.callCameraOff))
  ...
  if (minimized) {
    ...
    if (stage) {
      ...
      stage.addEventListener("mousedown", (e) => {
        ...
        startX = e.clientX
        startY = e.clientY
        origX = call.offsetLeft
        origY = call.offsetTop
...
window.addEventListener("pagehide", () => {
  try {
    ...
    if (activeCallHistoryId && state.user && state.call) {
      ...
      const endWhole =
        !isGroupCall || !callInitiatorId || callInitiatorId === state.user.id
...
async function inviteToActiveCall() {
  ...
  openFriendsPicker({
    ...
    onDone: async (ids) => {
      ...
      if (callPeers.size + ids.length > 5)
        return toast("This call is full (max 6 people)")
...
async function connectCall({ force = false } = {}) {
  ...
  try {
    ...
    const isVoice = Boolean(
      state.call?.voice || state.call?.callType === "voice",
    )
    ...
    if (!callPeers.size)
      setCallStatus(isVoice ? "calling" : "ringing", { force: true })
