...
const storeItems = [
  ...
  ["space", "Deep Space", "Sounds", sicon("planet"), 35, "A spacious ambient soundscape."],
  [
    "thunder",
    "Distant Thunder",
    "Sounds",
  ...
  ["double", "Double Dip", "Boosts", sicon("target"), 150, "Double one session reward."],
  ["quick", "Quick Start Pass", "Boosts", sicon("rocket"), 50, "Skip one setup step."],
  [
    "calm",
    "Calm Mode",
    "Boosts",
  ...
  ["spark", "Spark badge", "Badges", sicon("sparkle"), 60, "A bright profile detail."],
  [
    "creator",
    "Creator badge",
    "Badges",
  ...
  ["cat", "Study Cat avatar", "Avatars", sicon("cat"), 75, "A cosy study companion."],
  [
    "owl",
    "Wise Owl avatar",
    "Avatars",
  ...
  ["lotus", "Lotus avatar", "Avatars", sicon("lotus"), 90, "A calm profile identity."],
  [
    "crown",
...
const COIN_PACKS = [
  { id: "pack-10", coins: 10, price: 10, name: "Quick Sip", blurb: "A taste of momentum." },
  { id: "pack-25", coins: 25, price: 25, name: "Study Stash", blurb: "Fuel for the week." },
  { id: "pack-50", coins: 50, price: 50, name: "Scholar Vault", blurb: "For serious grinds.", tag: "Most popular" },
  { id: "pack-75", coins: 75, price: 75, name: "Deep Work Chest", blurb: "Lock in for the month." },
  { id: "pack-100", coins: 100, price: 100, name: "Legend Vault", blurb: "The full treasure room.", tag: "Best value" },
];
...
function openTopup(packId) {
  ...
  payBtn.onclick = async () => {
    ...
    if (!window.PaystackPop || !ready) {
      err.textContent = "Could not reach Paystack — check your connection and try again.";
      err.hidden = false;
    ...
    try {
      const handler = window.PaystackPop.setup({
        ...
        metadata: { coins: pack.coins, pack: pack.id, handle: state.profile.handle },
...
function rewardInfo(item) {
  ...
  if (id.includes("shield"))
    bits.push("Effect: arming it forgives your next missed day and protects your streak.");
  else if (id.includes("multipl"))
    bits.push("Effect: +25% coins on every session for 24 hours once activated.");
  else if (id.includes("double"))
    bits.push("Effect: doubles your next focus reward once armed.");
  else if (cat.includes("theme"))
    bits.push("Effect: recolors your whole app instantly — equip any time from your collection.");
  else if (cat.includes("sound"))
    bits.push("Effect: joins your sound studio mixer — layer it under the timer.");
  else if (cat.includes("badge"))
    bits.push("Effect: showcases on your profile next to your name.");
  else if (cat.includes("avatar"))
    bits.push("Effect: becomes your face everywhere — crews, leaderboards and chat included.");
  else
    bits.push("Effect: yours forever once purchased — re-equip any time from your collection.");
  if (ownsMine(item.id))
    bits.push("Status: already owned — find it in your collection below.");
  else if (item.season)
    bits.push("Limited season reward — leaves the shop when the countdown ends.");
  return bits.join(" ");
}
  ...
  state.owned = (state.owned || [])
    .map((o) => {
      if (o && (Array.isArray(o) || o.id == null)) {
        ...
        return { ...n, owner: o.owner || "me", boughtAt: o.boughtAt || Date.now() };
      }
      return o;
    })
    .filter((o) => o && o.id != null);
...
const EXCLUSIVE_REWARDS = [
  { id: "aurora-veil", name: "Aurora Veil theme", category: "Themes", emoji: sicon("sparkle"), price: 175, description: "Box-exclusive shifting aurora workspace.", tier: "Epic", exclusive: true },
  { id: "epic-scholar", name: "Epic Scholar badge", category: "Badges", emoji: sicon("trophy"), price: 165, description: "Box-exclusive mark of deep focus.", tier: "Epic", exclusive: true },
  { id: "celestial-gold", name: "Celestial Gold theme", category: "Themes", emoji: sicon("crown"), price: 300, description: "Box-exclusive radiant gold workspace.", tier: "Legendary", exclusive: true },
  { id: "starforge", name: "Starforge avatar", category: "Avatars", emoji: sicon("planet"), price: 280, description: "Box-exclusive cosmic profile identity.", tier: "Legendary", exclusive: true },
];
const MYSTERY_BOXES = {
  common: {
...
function boxItemsByRarity(rarity) {
  const pool = shopItems().filter(
    (i) => !ownsMine(i.id) || isStackable(i.id),
  );
...
const FREE_BOX_ODDS = { legendary: 0.0007, epic: 0.007, rare: 0.10, pityAfter: 30 };
...
function rollFreeBox(day) {
  ...
  if (tier === "Legendary") {
    ...
    if (cands.length && hash01(seed + "|kind") < 0.5) {
      ...
      return {
        tier,
        reward: { kind: "item", item, tier: "Legendary", emoji: item.emoji, name: item.name },
      };
    }
    const amount = 400 + Math.floor(hash01(seed + "|amt") * 401);
    return {
      tier,
      reward: { kind: "coins", amount, tier: "Legendary", emoji: sicon("coins"), name: `${amount} coins JACKPOT` },
    };
  }
  if (tier === "Epic") {
    ...
    if (cands.length && hash01(seed + "|kind") < 0.5) {
      ...
      return {
        tier,
        reward: { kind: "item", item, tier: "Epic", emoji: item.emoji, name: item.name },
      };
    }
    const amount = 150 + Math.floor(hash01(seed + "|amt") * 151);
    return {
      tier,
      reward: { kind: "coins", amount, tier: "Epic", emoji: sicon("coins"), name: `${amount} coins` },
    };
  }
  if (tier === "Rare") {
    ...
    if (cands.length && hash01(seed + "|kind") < 0.5) {
      ...
      return {
        tier,
        reward: { kind: "item", item, tier: "Rare", emoji: item.emoji, name: item.name },
      };
    }
    const amount = 60 + Math.floor(hash01(seed + "|amt") * 61);
    return {
      tier,
      reward: { kind: "coins", amount, tier: "Rare", emoji: sicon("coins"), name: `${amount} coins` },
    };
  }
  ...
  if (commons.length && hash01(seed + "|kind") < 0.3) {
    ...
    return {
      tier,
      reward: { kind: "item", item, tier: "Common", emoji: item.emoji, name: item.name },
    };
  }
  const amount = 8 + Math.floor(hash01(seed + "|amt") * 13);
  return {
    tier,
    reward: { kind: "coins", amount, tier: "Common", emoji: sicon("coin"), name: `${amount} coins` },
  };
}
...
function openBox(boxId) {
  ...
  const reveal = () => {
    ...
    if (fx.flash) stage.insertAdjacentHTML("beforeend", '<div class="freebox-flash"></div>');
    badge.hidden = false;
    ...
    sub.innerHTML = `Rarity: <span class="rarity-${reward.tier.toLowerCase()}">${reward.tier}</span> · ${reward.kind === "coins" ? "straight to your balance" : esc(reward.item.description || "")}`;
...
function openFreeBoxReveal() {
  ...
  const reveal = () => {
    ...
    sub.innerHTML =
      (r.kind === "coins" ? "Straight to your balance" : esc(r.item?.description || "")) +
  ...
  later(() => {
    ...
    title.textContent = rank >= 3 ? "Something mythic stirs…" : "Energy building…";
...
function recipientOptions() {
  return [
    { id: "me", icon: sicon("user"), name: "Buy for myself", sub: "Treat yourself" },
    { id: "friend", icon: sicon("gift"), name: "Gift to a Friend", sub: "Someone in your list" },
    { id: "user", icon: sicon("search"), name: "Gift to Another User", sub: "Search registered users" },
  ];
}
...
function giftPersonRow(p, selectedId) {
  ...
  return `<button type="button" class="gift-person${selectedId === p.id ? " selected" : ""}" data-gift-pick="${esc(p.kind + ":" + p.id)}"><span class="recipient-avatar sm">${avatar}</span><span class="recipient-txt"><strong>${esc(p.name)}</strong><small>${p.kind === "user" ? "Registered user" : "Friend"}</small></span>${selectedId === p.id ? '<span class="recipient-check" style="opacity:1;transform:none">' + sicon("check") + '</span>' : ""}</button>`;
}
...
function runGiftSearch(query) {
  ...
  if (!backendConfigured || !state.user || q.length < 2) {
    giftResults = [];
    giftSearching = false;
    box.innerHTML = `<p class="muted">${backendConfigured && state.user ? "Type 2+ letters to search the directory." : "Sign in to search every registered user."}</p>`;
    return;
  }
  giftSearching = true;
...
async function deliverGiftCloud(target, items, note) {
  ...
  try {
    ...
    const text = `${sicon("gift")} ${state.profile.name} sent you a gift: ${names}${note ? ` — “${note}”` : ""}`;
...
async function checkoutGiftFlow(message) {
  ...
  giftTarget = { type: target.kind === "user" ? "user" : "friend", id: target.id, name: target.name };
  // Snapshot what is in the cart BEFORE checkout() clears it, then let
  // checkout() run the real purchase (coins, ownership, history).
  const cartItems = shopItems().filter((i) => (state.selectedStore || []).includes(i.id));
  if (!cartItems.length) {
  ...
  const confirmed = await new Promise((resolve) =>
    checkout(() => resolve(true), () => resolve(false)),
  );
...
function renderStore() {
  ...
  $$("[data-box-buy]", t).forEach(
    (b) => (b.onclick = () => buyBox(b.dataset.boxBuy, itemQty("box:" + b.dataset.boxBuy))),
  );
  ...
  $$("[data-deal-buy]", t).forEach(
    (b) => (b.onclick = () => buyDeal(b.dataset.dealBuy, itemQty("deal:" + b.dataset.dealBuy))),
  );
    ...
    (b) =>
      (b.onclick = () => {
        ...
        state.equipped.theme =
          state.equipped.theme === item.id ? null : item.id;
        persist();
        pushUserSettings();
        applyEquippedTheme();
        shell();
  ...
  $$("[data-equip-avatar]", t).forEach(
    (b) =>
      (b.onclick = () => {
        ...
        shell();
        notify(
          state.equipped.avatar ? `${item.name} set` : "Avatar removed",
        );
      }),
  );
...
function checkout(onConfirmed, onCancelled) {
  ...
  confirmBox(
        ...
        if (
          ...
          state.messages[recipient] = [
            ...(state.messages[recipient] || []),
            {
              id: uid(),
              me: true,
              text: `${sicon("gift")} I sent you ${fresh.map((l) => (l.qty > 1 ? `${l.qty} × ${l.item.name}` : l.item.name)).join(", ")}${giftNote ? ` — “${giftNote}”` : ""}`,
              ts: Date.now(),
            },
          ];
        }
    ...
    { onCancel: () => { if (onCancelled) onCancelled(); } },
  );
}
