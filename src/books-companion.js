...
export function companionWordPanel(text, phrase, max = 3) {
  const p = String(phrase || "").trim().slice(0, 80);
...
export function renderCompanionPanel(side) {
  ...
  const render = () => {
    ...
    <div class="input-row" style="margin-top:10px"><input class="input" data-comp-q placeholder="Ask about this passage…" aria-label="Ask about this passage"><button type="button" class="primary" data-comp-ask>${compEditKey ? "Save" : "Ask"}</button></div>
    ...
    side.querySelectorAll("[data-comp-act]").forEach((b) => (b.onclick = () => {
    ...
    side.querySelectorAll("[data-comp-edit]").forEach((b) => (b.onclick = () => {
      compEditKey = b.dataset.compEdit;
      render();
      side.querySelector("[data-comp-q]")?.focus();
    }));
    side.querySelectorAll("[data-comp-del]").forEach((b) => (b.onclick = () => {
...
function answerCard(h) {
  return `<div class="companion-answer"><div class="section-row"><strong>${esc(h.title)}</strong>`
    ...
    + (h.query ? `<button type="button" class="icon-btn" data-comp-edit="${h.key}" title="Edit question" aria-label="Edit question">${sicon("memo")}</button>` : "")
...
function pushAnswer(mod, title, html, plain, meta = {}) {
  ...
  const key = meta.replaceKey || `${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
  const entry = {
    key, book: mod.bookId, title, html, plain: plain || title,
    act: meta.act || "", query: meta.query || "",
  };
  const at = compHistory.findIndex((h) => h.key === key);
  if (at >= 0) compHistory[at] = { ...compHistory[at], ...entry };
  else compHistory = [...compHistory.filter((h) => h.book === mod.bookId), entry].slice(-20);
  mod.rerender();
}
function runCompanion(mod, act, query, replaceKey = null) {
  const meta = { act, query: act === "ask" ? String(query || "") : "", replaceKey };
  const answer = (title, html, plain) => pushAnswer(mod, title, html, plain, meta);
  const text = mod.contextText || "";
  if (!text.trim() && act !== "cards") {
    answer( "No text to work with", "<p class=\"muted\">Open a text book section first.</p>", "No text");
    return;
  }
  if (act === "explain" || act === "summarize") {
    const top = companionSummary(text, 3);
    if (!top.length) return answer( "Summary", "<p class=\"muted\">Not enough text here to summarize.</p>", "none");
    answer( act === "explain" ? "In simpler terms" : "Summary",
    ...
    const phrase = String(mod.askPhrase || "").trim().slice(0, 80);
    if (phrase) {
      const panel = companionWordPanel(text, phrase, 3);
      if (!panel.count) {
        return answer( `“${phrase}”`,
          `<p class="muted">That phrase does not appear elsewhere in ${esc(companionContextLabel(mod.ctxShort || mod.ctx))}.</p>`, "no match");
      }
      answer( `“${panel.phrase}” · from this book`,
        `<p class="muted">On-device definition — ${panel.count}× in this ${mod.ctxKind === "book" ? "book" : "passage"}. A cloud dictionary is not configured, so meanings come from context:</p>`
    ...
    if (!terms.length) return answer( "Key terms", "<p class=\"muted\">No standout terms in this passage.</p>", "none");
    answer( "Key terms here",
    ...
    if (!terms.length) return answer( "Key terms", "<p class=\"muted\">No standout terms found.</p>", "none");
    answer( "Key terms",
    ...
    if (!cards.length) return answer( "Flashcards", "<p class=\"muted\">Highlight passages first — each becomes a flashcard.</p>", "none");
    answer( `Flashcards (${cards.length})`,
    ...
    if (!scored.length) {
      answer( "No direct match", `<p class="muted">I could not find that in ${esc(companionContextLabel(mod.ctxShort || mod.ctx))}. Try the Key terms or Summary actions, or rephrase with words from the passage.</p>`, "no match");
      return;
    }
