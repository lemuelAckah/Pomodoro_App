...
import {
  state, $, $$, uid, get, save, esc, sicon, persist, notify,
...
function collectSearch(q) {
  ...
  allGroups().forEach((g) => {
    if (!q || `${g.name} ${(g.tags || []).join(" ")}`.toLowerCase().includes(q)) {
  ...
  window.addEventListener(
    ...
    (e) => {
      try {
        ...
        if (e.target?.closest?.(".friend-pick.drop, .modal, [data-picker]")) return;
        document
          .querySelectorAll(".friend-pick.drop, .post-menu.chat-menu, .post-menu[data-post-pop], [data-picker]")
          .forEach((el) => {
            el.hidden = true;
          });
...
if (!window.__sfKeysBound) {
  ...
  if ((e.key === "r" || e.key === "R") && !e.ctrlKey && !e.metaKey && !e.altKey) {
    if ($("#search-palette") || $("#status-viewer") || $("#focus-view")) return;
    if (state.tab === "timer") {
      e.preventDefault();
      resetFocusSession();
    }
...
if (!window.__sfStatusBound) {
  ...
  document.addEventListener("contextmenu", (e) => {
    ...
    e.preventDefault();
    askDeleteStatus({ id: chip.dataset.storyView, key: "story:" + chip.dataset.storyView, type: "story" });
  });
}
...
export { searchHits, openSearch, closeSearch, collectSearch, renderSearchResults, runSearchHit, shell, bindShell, render, showUpdateToast };
