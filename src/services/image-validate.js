...
export function imageKindFromMime(mime) {
  return MIME_KINDS[String(mime || "").toLowerCase().trim()] || null;
}
...
function recordImageDiag(rec) {
  try {
    ...
    if (typeof window !== "undefined") {
      ...
      console.debug("[sf-image]", entry.verdict, entry.caller || "-",
        JSON.stringify({ name: entry.name, type: entry.type, size: entry.size, magic: entry.magicHex }));
    }
  } catch {
...
function hexHead(u8, n = 16) {
  try {
    return [...u8.slice(0, n)].map((b) => b.toString(16).padStart(2, "0").toUpperCase()).join(" ");
  } catch {
    return "";
  }
}
export async function validateImageFile(file, maxBytes, caller = "") {
  const base = { caller, name: file?.name, type: file?.type, size: file?.size, lastModified: file?.lastModified };
  ...
  if (!(file.size > 0)) return fail("reject:empty", "That image looks empty — pick another");
  ...
  if (!mimeKind && !extKind) return fail("reject:unlabeled", IMAGE_FORMAT_ERROR, signals);
  if (magicKind) {
