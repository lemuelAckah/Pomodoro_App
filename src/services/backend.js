...
export async function deleteMyBackendData() {
  ...
  try {
    const { data: owned } = await supabase.from("groups").select("id").eq("owner_id", uid);
    const avatars = supabase.storage.from("studyflow-groups");
    for (const g of owned || []) {
      const listed = await avatars.list(`groups/${g.id}`).catch(() => null);
      const files = (listed?.data || []).map((e) => `groups/${g.id}/${e.name}`).filter((p) => !p.endsWith("/"));
...
const bookSyncRow = (userId, b) => ({
  ...
  tags: Array.isArray(b.tags) ? b.tags.slice(0, 12).map((x) => String(x).slice(0, 40)) : [],
  ...
  client_updated_at: b.updatedAt ? new Date(b.updatedAt).toISOString() : new Date().toISOString(),
});

export async function cloudUpsertBook(book) {
  const { userId, error } = await bookOwnerId();
  if (error) return { data: null, error };
  return supabase.from("books").upsert(bookSyncRow(userId, book), { onConflict: "id" }).select().single();
}
...
export async function cloudUpsertBooks(books) {
  ...
  const { error: e } = await supabase
    .from("books")
    .upsert(books.map((b) => bookSyncRow(userId, b)), { onConflict: "id" });
...
export async function cloudListBooks() {
  ...
  const { data, error: e } = await supabase
    .from("books")
    .select("id,title,author,description,category,tags,isbn,publisher,published_year,page_count,word_count,cover_path,file_path,file_type,file_size,fingerprint,visibility,allow_download,created_at,client_updated_at")
    .eq("owner_id", userId)
  ...
  return {
    data: (data || []).map((r) => ({
      ...
      category: r.category, tags: r.tags || [], isbn: r.isbn || "",
      publisher: r.publisher || "", year: r.published_year ?? null,
      pageCount: r.page_count ?? null, wordCount: r.word_count ?? null,
      coverPath: r.cover_path || "", filePath: r.file_path || "",
      fileType: r.file_type || "", fileSize: r.file_size || 0,
      fingerprint: r.fingerprint || "",
      visibility: "private", allowDownload: Boolean(r.allow_download),
...
export async function listBookNotes(bookId) {
  const { userId, error } = await bookOwnerId();
  if (error) return { data: [], error };
  return supabase.from("book_notes").select("*").eq("user_id", userId).eq("book_id", bookId).order("created_at", { ascending: true });
}

export async function addBookNote(bookId, { kind, label, locator, body }) {
  const { userId, error } = await bookOwnerId();
  if (error) return { data: null, error };
  if (!String(body || "").trim()) return { data: null, error: new Error("Note is empty") };
  return supabase.from("book_notes").insert({
    user_id: userId, book_id: bookId,
...
export async function updateBookNote(id, patch) {
  ...
  if (typeof patch.body === "string") {
    if (!patch.body.trim()) return { data: null, error: new Error("Note is empty") };
    clean.body = patch.body.slice(0, 4000);
  }
  if (typeof patch.label === "string") clean.label = patch.label.slice(0, 200);
  if (typeof patch.locator === "string") clean.locator = patch.locator.slice(0, 2000);
  if (!Object.keys(clean).length) return { data: null, error: new Error("Nothing to update") };
  clean.updated_at = new Date().toISOString();
  return supabase.from("book_notes").update(clean).eq("id", id).eq("user_id", userId).select().single();
}
...
export async function listPublicGroups(limit = 200) {
  ...
  const res = await supabase
    .from("groups")
    .select("id,owner_id,name,description,logo,focus_topics,visibility,avatar_path,group_memberships(count)")
...
export function subscribeToUserState(userId, onChange) {
  ...
  const channel = supabase
    ...
    .on(
      ...
      (payload) => onChange({ state: payload.new?.state ?? null, version: payload.new?.version ?? 0 }),
    )
    .subscribe();
...
function rewardsUnavailable() {
  return { data: null, error: new Error("Rewards need the Phase 4 database update — run migration 012 in Supabase") };
}
...
export function friendlyRewardError(error) {
  ...
  if (/INVALID_REWARD|REWARD_UNAVAILABLE/i.test(msg)) return "That reward is no longer available.";
  ...
  if (/RECIPIENT_NOT_FOUND/i.test(msg)) return "Couldn't find that user anymore.";
  if (/INVALID_RECIPIENT/i.test(msg)) return "You can't send a gift there.";
  if (/NOT_OWNED/i.test(msg)) return "You don't own that anymore — syncing your collection.";
  if (/ALREADY_CLAIMED/i.test(msg)) return "Already claimed — your next box lands after midnight UTC.";
  if (/INVALID_AMOUNT|INVALID_REASON|INVALID_REF|INVALID_DAY|INVALID_BOX|INVALID_ACHIEVEMENT/i.test(msg))
...
export async function giftRewardRpc(rewardId, qty, recipientId, idempotencyKey) {
...
export const loadCoinTransactions = () => rewardsRead("coin_transactions", "created_at");
...
export const loadUserAchievements = () => rewardsRead("user_achievements", "unlocked_at");
export const loadBoxOpenings = () => rewardsRead("mystery_box_openings", "opened_at");
...
export async function cloudListMusicTracks() {
  ...
  const { data, error: e } = await supabase
    .from("music_tracks")
    .select("id,title,artist,album,file_name,mime,size_bytes,duration_sec,fingerprint,client_created_at,client_updated_at")
    .eq("user_id", userId);
  if (e) return { data: null, error: e };
  return {
    data: (data || []).map((r) => ({
...
export async function cloudUpsertMusicTracks(tracks) {
  ...
  const { error: e } = await supabase
    .from("music_tracks")
    .upsert(tracks.map((t) => trackRow(userId, t)), { onConflict: "user_id,id" });
  return e ? { data: null, error: e } : { data: true, error: null };
}
...
export async function cloudListPlaylists() {
  const { userId, error } = await requireUserId();
  if (error) return { data: null, error };
  const [{ data: lists, error: e1 }, { data: links, error: e2 }] = await Promise.all([
    supabase
...
export async function cloudUpsertPlaylists(playlists) {
  ...
  for (const p of playlists) {
    ...
    const ids = [...new Set((p.songIds || []).map((s) => String(s).slice(0, 64)))];
    if (!ids.length) continue;
    const ins = await supabase.from("music_playlist_tracks").insert(
      ids.map((tid, i) => ({ user_id: userId, playlist_id: pid, track_id: tid, position: i })),
    );
...
export async function getPublicProfiles(ids) {
  if (!supabase)
    return { data: [], error: new Error("Backend is not configured") };
  const list = [...new Set((ids || []).map((v) => String(v)).filter(Boolean))].slice(0, 100);
...
export async function groupLeave(groupId) {
  const { error: authError } = await requireUserId();
  if (authError) return { data: null, error: authError };
  const res = await supabase.rpc("sf_group_leave", { p_group_id: cleanId(groupId) });
  if (res.error && isMissingRpc(res.error)) return phase7Unavailable();
  return res;
}
...
export async function groupDelete(groupId) {
  const { error: authError } = await requireUserId();
  if (authError) return { data: null, error: authError };
  const res = await supabase.rpc("sf_group_delete", { p_group_id: cleanId(groupId) });
  if (res.error && isMissingRpc(res.error)) return phase7Unavailable();
  return res;
}
...
export async function groupUpdate(groupId, patch = {}) {
  ...
  const res = await supabase.rpc("sf_group_update", {
    ...
    p_topics: Array.isArray(patch.topics) ? patch.topics.map((t) => String(t)) : null,
...
export async function groupRole(groupId) {
  ...
  const res = await supabase.rpc("sf_group_role", { p_group_id: cleanId(groupId) });
  if (res.error && isMissingRpc(res.error)) return phase7Unavailable();
  return res;
}
...
export async function getMyGroups() {
  ...
  const res = await supabase
    .from("group_memberships")
    .select("role,joined_at,muted_at,groups(id,owner_id,name,description,logo,focus_topics,visibility,avatar_path,created_at)")
    .eq("user_id", userId)
  ...
  return {
    data: (res.data || []).filter((r) => r.groups).map((r) => ({ ...r.groups, myRole: r.role, joinedAt: r.joined_at, mutedAt: r.muted_at })),
    error: null,
  };
}
...
export async function getGroupMembers(groupId) {
  ...
  return {
    data: rows.map((r) => ({ ...r, ...(byId.get(r.user_id) || { id: r.user_id }) })),
    error: null,
  };
}
...
export async function loadConversation({ groupId, recipientId, limit = 60, before = null } = {}) {
  ...
  if (groupId) {
    ...
    q = q.or(`and(sender_id.eq.${userId},recipient_id.eq.${recipientId}),and(sender_id.eq.${recipientId},recipient_id.eq.${userId})`);
  } else {
...
export async function respondFriendRequest(requestId, accept) {
  ...
  return supabase.from("friendships").delete().eq("id", requestId).eq("friend_id", userId);
}
...
export async function notifyEvent({ type, eventKey, entityId = null, metadata = {}, groupId = null, userId = null } = {}) {
  const { error: authError } = await requireUserId();
  if (authError) return { data: null, error: authError };
...
export async function createStory({ text = "", mediaPath = null, kind = "text", visibility = "connections" } = {}) {
  const { userId, error } = await requireUserId();
  if (error) return { data: null, error };
  const clean = String(text || "").slice(0, 500);
  if (!clean.trim() && !mediaPath)
    return { data: null, error: new Error("Write something or add a photo first") };
  if (!["public", "connections", "private"].includes(visibility)) visibility = "connections";
  if (!["text", "image"].includes(kind)) kind = mediaPath ? "image" : "text";
  return supabase
    .from("stories")
    .insert({
      user_id: userId,
    ...
    .select("id,user_id,kind,text,media_path,visibility,expires_at,created_at")
    .single();
}
...
export async function deleteStory(storyId) {
  const { userId, error } = await requireUserId();
  if (error) return { data: null, error };
  return supabase.from("stories").delete().eq("id", storyId).eq("user_id", userId);
}
...
function cacheSignedUrl(bucket, path, url, expiresIn = 3600) {
  ...
  signedUrlCache.set(key, { url, exp: Date.now() + Math.max(60, expiresIn - 120) * 1000 });
}
async function cachedSignedUrl(bucket, path, expiresIn = 3600) {
  if (!supabase || !path) return { data: null, error: new Error("Storage is not configured") };
  const key = `${bucket}:${path}`;
  const hit = signedUrlCache.get(key);
  if (hit && hit.exp > Date.now()) return { data: { signedUrl: hit.url }, error: null };
  signedUrlCache.delete(key);
  const res = await supabase.storage.from(bucket).createSignedUrl(path, expiresIn);
  if (!res.error && res.data?.signedUrl) cacheSignedUrl(bucket, path, res.data.signedUrl, expiresIn);
  return res;
}
...
function extFor(file) {
  ...
  const t = byType[String(file.type || "").toLowerCase().trim()];
  if (t) return t;
  const ext = String(file.name || "").split(/[?#]/)[0].split(".").pop()
...
export async function uploadGroupAvatar(groupId, file) {
  const { userId, error } = await requireUserId();
  if (error) return { data: null, error };
  if (!userId) return { data: null, error: new Error("Sign in to change the group avatar") };
  const bad = await validateImageFile(file, GROUP_AVATAR_MAX, "upload-group-avatar");
  ...
  const role = await supabase.rpc("sf_group_role", { p_group_id: gid }).catch(() => ({ data: null }));
  if (role?.data !== "owner" && role?.data !== "admin")
    return { data: null, error: new Error("Only the group owner or an admin can change the avatar") };
...
export async function uploadStoryPhoto(file) {
  const { userId, error } = await requireUserId();
  if (error) return { data: null, error };
  const bad = await validateImageFile(file, STORY_PHOTO_MAX, "upload-story-photo");
  if (bad) return { data: null, error: new Error(bad) };
  ...
  try {
    ...
    console.debug("[sf-story-upload] storage-result", {
      ok: !e, path, contentType: file.type || "(none)",
  ...
  } catch { /* diagnostics must never break uploads */ }
  if (res.error) return { data: null, error: res.error };
  return { data: { path }, error: null };
}
...
async function storageDiag() {
  ...
  try {
    ...
    const summarize = (b) => b ? {
      id: b.id, name: b.name, public: b.public, owner: b.owner || null,
    ...
    report.steps.buckets = {
      ...
      error: buckets.error ? String(buckets.error.message || buckets.error) : null,
    };
    ...
    if (!up.error) {
      ...
      report.steps.cleanup = { ok: !del.error, error: del.error ? String(del.error.message || del.error) : null };
    }
    ...
    const av = await supabase.storage.from("studyflow-avatars").upload(avPath, blob, {
      contentType: "image/png",
      upsert: false,
    });
    report.steps.avatarControl = {
      ok: !av.error, path: avPath,
    ...
    if (!av.error) {
      const del = await supabase.storage.from("studyflow-avatars").remove([avPath]);
      report.steps.avatarCleanup = { ok: !del.error, error: del.error ? String(del.error.message || del.error) : null };
    }
  } catch (err) {
