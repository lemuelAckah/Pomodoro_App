...
export async function flushRewardOutbox() {
  ...
  for (const event of list) {
    ...
    if (res.error) {
      ...
      if (
        /failed to fetch|networkerror|network request failed|load failed|NOT_SIGNED_IN/i.test(
          msg,
        )
      )
        return
...
export async function secureEarn({ amount, reason, refKey, metadata = {} }) {
  ...
  if (res.error) {
    ...
    return {
      ok: false,
      error: friendlyRewardError(res.error),
      balance: state.coins,
    }
  }
  serverCoins = res.data.balance
...
const SPEND_OFFLINE_MSG =
  "You are offline. Reconnect to the internet to make purchases."
...
export async function securePurchase({ rewardId, qty = 1, fallbackPrice = 0 }) {
  ...
  if (res.error) {
    ...
    return {
      ok: false,
      error: offline ? SPEND_OFFLINE_MSG : friendlyRewardError(res.error),
      balance: state.coins,
    }
  }
  serverCoins = res.data.balance
...
export async function secureGift({
  rewardId,
  qty = 1,
  recipientId,
  fallbackPrice = 0,
...
export async function secureConsume({ rewardId, qty = 1, sellBack = false }) {
  ...
  const res = await consumeRewardRpc(
    rewardId,
    qty,
    sellBack,
    sellBack ? key : null,
...
export async function secureUnlock(achievementId) {
  if (!cloudRewards())
    return { ok: true, local: true, unlocked: true, reward: 0 }
  const res = await unlockAchievementRpc(
    achievementId,
...
export async function secureStreak(refKey) {
  ...
  try {
    ...
    state.streak = {
      count:
        Number.isFinite(served) && served >= 1
          ? served
    ...
    if (res.data.longest > (state.bestStreak || 0))
      state.bestStreak = res.data.longest
...
export async function pullRewards() {
  ...
  if (!bal.error && Number.isFinite(+bal.data)) {
    ...
    if (remote === 0 && state.coins > 0 && !get("sf-coins-migrated", false)) {
      ...
      serverCoins = remote
      save("sf-balance", serverCoins)
      if (!get("sf-coins-migrated", false) && remote > 0)
        save("sf-coins-migrated", "1")
    }
  ...
  if (!purch.error)
    cache("sf-purchases-cloud", (purch.data || []).slice(0, 100))
