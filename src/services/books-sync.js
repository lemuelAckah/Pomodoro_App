...
import { cloudUpsertBooks, cloudListBooks, deleteBook } from "./backend.js"
...
export async function pullBooks(adopt) {
  ...
  try {
    ...
    if (res.error) throw res.error
    const byId = new Map(
      (state.books || []).filter(Boolean).map((b) => [b.id, b]),
    )
    ...
    for (const cloud of res.data || []) {
      ...
      const cloudTs = cloud.clientUpdatedAt
        ? new Date(cloud.clientUpdatedAt).getTime()
        : 0
      if (cloudTs > Number(local.updatedAt || 0)) {
        byId.set(cloud.id, {
          ...local,
          ...cloud,
          fileKey: local.fileKey || local.id,
        })
        changed = true
      }
    }
