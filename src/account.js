...
function openProfile() {
  ...
  modal.innerHTML = `<div class="modal profile-modal"><div class="eyebrow">Your account</div><h2>Profile &amp; preferences</h2><p class="muted">${backendConfigured ? "Cloud sync is available for this workspace." : "Local mode is active. Add Supabase environment keys to enable accounts and cloud sync."}</p><div class="profile-grid">
...
function renderAccount() {
  ...
  target.innerHTML = `<div class="account-page"><div class="account-hero"><div><div class="eyebrow">StudyFlow identity</div><h1>${user ? "Your account, your space." : "A calmer way to sign in."}</h1><p class="lede">${user ? "Manage your profile, privacy, and connected sessions from one secure place." : "Join your focused workspace and keep your progress with you across devices."}</p></div><div class="account-orbit"><span>◷</span><i></i><b></b></div></div>${completenessMarkup()}<div class="account-layout"><div class="card auth-card">${user ? accountSignedInMarkup(user) : accountAuthMarkup()}</div><aside class="account-aside"><div class="eyebrow">Built for trust</div><h2>Private by default.</h2><p class="muted">Your profile controls, cloud data, and session access are managed here. Authentication is handled by Supabase Auth when configured.</p><div class="security-list"><div><span>${sicon("check")}</span><p><strong>Email verification</strong><small>Confirm ownership before cloud access.</small></p></div><div><span>${sicon("check")}</span><p><strong>Global sign-out</strong><small>End sessions on every device.</small></p></div><div><span>${sicon("check")}</span><p><strong>Privacy controls</strong><small>Choose who can discover your profile.</small></p></div></div></aside></div>${dataMarkup()}${privacyCard()}</div>`;
  bindAccount(target);
}
