...
function handleFieldMarkup(id, value, hint) {
  return `<label class="field-label">Username<span class="handle-row"><input class="input" id="${id}" value="${esc(value || "")}" placeholder="pick_a_name" autocomplete="off" autocapitalize="off" spellcheck="false"><button type="button" class="dice-btn" data-dice-for="${id}" title="Roll a random username" aria-label="Roll a random username">${sicon("dice")}</button></span>${hint ? `<small class="muted">${esc(hint)}</small>` : ""}</label>`;
}
...
function completenessMarkup() {
  ...
  return `<div class="card" style="margin-bottom:18px"><div class="section-row"><h2>Profile strength</h2><span class="tag">${pct}%</span></div><div class="complete-track"><span class="complete-fill" style="width:${pct}%"></span></div>${pct >= 100 ? '<p class="muted" style="margin-top:10px">Profile complete — recommendations and buddy matching run at full power.</p>' : `<p class="muted" style="margin-top:10px">Add your ${esc(missing.slice(0, 2).join(" and "))} to unlock better matches.</p><div class="complete-missing">${missing.map((m) => `<button type="button" class="ghost complete-chip" data-complete-profile>${esc(m)}</button>`).join("")}</div>`}</div>`;
}
...
function privacyCard() {
  ...
  return `<div class="card" style="margin-top:18px"><div class="section-row"><h2>Privacy &amp; guidelines</h2><span class="tag">${ok ? "accepted" : "review"}</span></div><p class="muted">${ok ? `You accepted version ${esc(a.version)}${a.at ? ` on ${new Date(a.at).toLocaleDateString()}` : ""}. The house rules live here whenever you need them.` : "Read the do's and don'ts of StudyFlow — your data rights, safety tools and community rules in one place."}</p><div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap"><button type="button" class="ghost" data-privacy-open>Read guidelines</button><button type="button" class="ghost" data-privacy-pdf>${sicon("download")} Save as PDF</button></div></div>`;
}
...
function appearanceCard() {
  return `<div class="card"><div class="section-row"><h2>Appearance</h2><span class="tag">${state.night ? "night" : "day"}</span></div><p class="muted">Switch the whole app between Day and Night mode. Your choice saves instantly and follows your account.</p><div class="seg-row" role="group" aria-label="Color mode"><button type="button" class="seg${!state.night ? " on" : ""}" data-mode-set="day">Day</button><button type="button" class="seg${state.night ? " on" : ""}" data-mode-set="night">Night</button></div></div>`;
}
function techCheckCard() {
  const tc = state.techCheck;
  if (tc && tc.done && (tc.top || []).length) {
    const first = techInfo(tc.top[0]);
    return `<div class="card"><div class="section-row"><h2>Study technique profile</h2><span class="tag">your check</span></div><p class="muted">Top match: <strong>${first ? esc(first[1]) : "—"}</strong>${tc.date ? ` · checked ${new Date(tc.date).toLocaleDateString()}` : ""}. Your Techniques tab highlights these picks.</p><button type="button" class="ghost" data-tc-retake>Retake Technique Check</button></div>`;
  }
