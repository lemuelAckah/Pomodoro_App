...
export function parseZipCentralDirectory(bytes) {
  ...
  for (let i = len - 22; i >= scanStart; i--) {
    if (readU32(view, i) === ZIP_EOCD_SIG) {
      eocd = i
      break
    }
  }
  ...
  for (let n = 0; n < count; n++) {
    if (readU32(view, off) !== ZIP_CENTRAL_SIG)
...
function domParse(xml, type) {
  ...
  if (doc.querySelector("parsererror"))
    throw new Error("Unparseable EPUB metadata")
  return doc
}
...
export async function parseEpub(blob) {
  ...
  if (
    ...
    throw new Error(
      "EPUB reading needs a modern browser (DOMParser + DecompressionStream)",
    )
  }
  ...
  const labels = {}
  try {
    const navItem = Object.values(items).find((it) =>
      (it.props || "").split(/\s+/).includes("nav"),
    )
    if (navItem) {
        ...
        const toc =
          navs.find(
            (n) =>
              ...
              (n.getAttribute("type") || "").includes("toc"),
          ) || navs[0]
        if (toc) {
          for (const a of toc.getElementsByTagName("a")) {
            ...
            if (key && !(key in labels))
              labels[key] = (a.textContent || "").trim().slice(0, 120)
          }
        }
      }
    } else {
          ...
          for (const np of ncx.getElementsByTagName("navPoint")) {
            const t =
              np
                ...
                .slice(0, 120) || ""
            const src =
              np.getElementsByTagName("content")[0]?.getAttribute("src") || ""
  ...
  try {
    const coverItem =
      ...
      Object.values(items).find((it) =>
        /cover[^/]*\.(jpe?g|png|webp|gif)$/i.test(it.href),
      )
    if (coverItem) {
  ...
  for (const ref of spineRefs) {
    ...
    try {
      html = textDecoder().decode(
        await inflateZipEntry(bytes, byName.get(path)),
      )
    } catch {
  ...
  if (!chapters.some((c) => c.sections.length))
...
function chapterToSections(html, path) {
  ...
  for (const bad of doc.body.querySelectorAll(
