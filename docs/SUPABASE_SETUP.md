...
**Required for cross-user messaging:** run [023_dm_delivery_fixes.sql](supabase/migrations/023_dm_delivery_fixes.sql). It fixes three production faults: (1) friend requests silently failing to insert (the requester's upsert returned a row the insert policy rejected), (2) one-directional DM inserts (if A sent the request, B could not reply — every reply was rejected by RLS), and (3) adds a server-validated `sf_direct_message` RPC the app uses for DMs. Without this migration, messages sent between two users will not appear for the recipient.
